# UnifySpecSnapshot

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, Bool, Bool)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, Int, Int)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, Int, Int)
```

### Output
```
<A: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, Int, Int)
```

### Output
```
<A: !, B: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C: !>, Int, Int)
```

### Output
```
<A: !, B: !, C: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Int, fun(Int) -> Int)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Bool, fun(Int) -> Bool)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> fun(Bool) -> Bool, fun(Int) -> fun(Bool) -> Bool)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Int, fun(Int) -> Bool)
```

### Output
```
<>
```

### Errors
- (0:22-0:25) Test failed because `Int` is not a `Bool`.
  - (0:39-0:43): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Int, fun(Bool) -> Int)
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:31-0:35): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Int, fun(Bool) -> Bool)
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:31-0:35): `Bool`
- (0:22-0:25) Test failed because `Int` is not a `Bool`.
  - (0:40-0:44): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> fun(Int) -> Int, fun(Bool) -> fun(Bool) -> Bool)
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:43-0:47): `Bool`
- (0:26-0:29) Test failed because `Int` is not a `Bool`.
  - (0:56-0:60): `Bool`
- (0:34-0:37) Test failed because `Int` is not a `Bool`.
  - (0:65-0:69): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(fun(Int) -> Int) -> Int, fun(fun(Bool) -> Bool) -> Bool)
```

### Output
```
<>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:47-0:51): `Bool`
- (0:26-0:29) Test failed because `Int` is not a `Bool`.
  - (0:56-0:60): `Bool`
- (0:34-0:37) Test failed because `Int` is not a `Bool`.
  - (0:65-0:69): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, Int, fun(Int) -> Int)
```

### Output
```
<>
```

### Errors
- (0:10-0:13) Test failed because `Int` is not a function.
  - (0:15-0:30): function

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, fun(Int) -> Int, Int)
```

### Output
```
<>
```

### Errors
- (0:10-0:25) Test failed because a function is not an `Int`.
  - (0:27-0:30): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, A, Bool)
```

### Output
```
<A = Int>
```

### Errors
- (0:17-0:18) Test failed because `Int` is not a `Bool`.
  - (0:11-0:14): `Int`
  - (0:20-0:24): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, Bool, A)
```

### Output
```
<A = Int>
```

### Errors
- (0:17-0:21) Test failed because `Bool` is not an `Int`.
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, A, Bool)
```

### Output
```
<A: Int>
```

### Errors
- (0:16-0:17) Test failed because `Int` is not a `Bool`.
  - (0:10-0:13): `Int`
  - (0:19-0:23): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, Bool, A)
```

### Output
```
<A: Int>
```

### Errors
- (0:16-0:20) Test failed because `Bool` is not an `Int`.
  - (0:10-0:13): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A = B>, A, Int)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A = B>, Int, A)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A = B>, A, Bool)
```

### Output
```
<B = Int, A = B>
```

### Errors
- (0:24-0:25) Test failed because `Int` is not a `Bool`.
  - (0:11-0:14): `Int`
  - (0:27-0:31): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A = B>, Bool, A)
```

### Output
```
<B = Int, A = B>
```

### Errors
- (0:24-0:28) Test failed because `Bool` is not an `Int`.
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A: B>, A, Int)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A: B>, Int, A)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A: B>, A, Bool)
```

### Output
```
<B = Int, A: B>
```

### Errors
- (0:23-0:24) Test failed because `Int` is not a `Bool`.
  - (0:11-0:14): `Int`
  - (0:26-0:30): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = Int, A: B>, Bool, A)
```

### Output
```
<B = Int, A: B>
```

### Errors
- (0:23-0:27) Test failed because `Bool` is not an `Int`.
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A = B>, A, Int)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A = B>, Int, A)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A = B>, A, Bool)
```

### Output
```
<B: Int, A = B>
```

### Errors
- (0:23-0:24) Test failed because `Int` is not a `Bool`.
  - (0:10-0:13): `Int`
  - (0:26-0:30): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A = B>, Bool, A)
```

### Output
```
<B: Int, A = B>
```

### Errors
- (0:23-0:27) Test failed because `Bool` is not an `Int`.
  - (0:10-0:13): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A: B>, A, Int)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A: B>, Int, A)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A: B>, A, Bool)
```

### Output
```
<B: Int, A: B>
```

### Errors
- (0:22-0:23) Test failed because `Int` is not a `Bool`.
  - (0:10-0:13): `Int`
  - (0:25-0:29): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: Int, A: B>, Bool, A)
```

### Output
```
<B: Int, A: B>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:10-0:13): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, A)
```

### Output
```
<A: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, A)
```

### Output
```
<A: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !>, A, A)
```

### Output
```
<A = !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, B, C)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, C, B)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, A, B)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, A, C)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, B, A)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, C, A)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, B, A)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, B, B)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = A, C = B>, C, C)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, B, C)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, C, B)
```

### Output
```
<A: !, B = <Z: !> A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, A, B)
```

### Output
```
<A: !, B = A, C = <Z: !> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, A, C)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, B, A)
```

### Output
```
<A: !, B = A, C = <Z: !> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, C, A)
```

### Output
```
<A: !, B = A, C = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, B, A)
```

### Output
```
<A: !, B = A, C = <Z: !> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, B, B)
```

### Output
```
<A: !, B = <Z: !> A, C = <Z: !> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !,  B = <Z: !> A, C = <Z: !> B>, C, C)
```

### Output
```
<A: !, B = <Z: !> A, C = <Z: !> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !>, A, Int)
```

### Output
```
<A = !>
```

### Errors
- (0:15-0:16) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`
  - (0:18-0:21): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !>, Int, A)
```

### Output
```
<A = !>
```

### Errors
- (0:15-0:18) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A: B>, A, Int)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A: B>, Int, A)
```

### Output
```
<B = Int, A: B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A = B>, A, Int)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A = B>, Int, A)
```

### Output
```
<B = Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = !, A: B>, A, Int)
```

### Output
```
<B = !, A: B>
```

### Errors
- (0:21-0:22) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = !, A: B>, Int, A)
```

### Output
```
<B = !, A: B>
```

### Errors
- (0:21-0:24) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = !, A = B>, A, Int)
```

### Output
```
<B = !, A = B>
```

### Errors
- (0:22-0:23) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`
  - (0:25-0:28): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B = !, A = B>, Int, A)
```

### Output
```
<B = !, A = B>
```

### Errors
- (0:22-0:25) Test failed because `!` is more general than `Int`.
  - (0:11-0:12): `!`

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A = fun(B) -> B>, A, B)
```

### Output
```
<B: !, A = fun(B) -> B>
```

### Errors
- (0:31-0:32) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, A = fun(B) -> B>, B, A)
```

### Output
```
<B: !, A = fun(B) -> B>
```

### Errors
- (0:31-0:32) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, C = fun(B) -> B, A = C>, A, B)
```

### Output
```
<B: !, C = fun(B) -> B, A = C>
```

### Errors
- (0:38-0:39) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, C = fun(B) -> B, A = C>, B, A)
```

### Output
```
<B: !, C = fun(B) -> B, A = C>
```

### Errors
- (0:38-0:39) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, C = fun(B) -> B, A = fun(C) -> C>, A, B)
```

### Output
```
<B: !, C = fun(B) -> B, A = fun(C) -> C>
```

### Errors
- (0:48-0:49) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<B: !, C = fun(B) -> B, A = fun(C) -> C>, B, A)
```

### Output
```
<B: !, C = fun(B) -> B, A = fun(C) -> C>
```

### Errors
- (0:48-0:49) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, B, A)
```

### Output
```
<B: !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, A, fun(B) -> B)
```

### Output
```
<B: !, A = fun(B) -> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, fun(B) -> B, A)
```

### Output
```
<B: !, A = fun(B) -> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X>, A, fun(Int) -> Bool)
```

### Output
```
<A: fun<X>(X) -> X>
```

### Errors
- (0:27-0:28) Test failed because `Int` is not a `Bool`.
  - (0:34-0:37): `Int`
  - (0:42-0:46): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun<X>(X) -> X>
```

### Errors
- (0:28-0:29) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:11-0:25): `fun<X>(X) -> X`
  - (0:31-0:46): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X>, A, fun(Int) -> Bool)
```

### Output
```
<A = fun<X>(X) -> X>
```

### Errors
- (0:28-0:29) Test failed because `Int` is not a `Bool`.
  - (0:35-0:38): `Int`
  - (0:43-0:47): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B: fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, B, A)
```

### Output
```
<B: fun<Z>(Z) -> Z, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, B, A)
```

### Output
```
<B: fun<Z>(Z) -> Z, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y, Z>(Y) -> Z>, B, A)
```

### Output
```
<B = fun<Z>(Z) -> Z, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>
```

### Errors
- (0:50-0:51) Test failed because `fun<Y>(Y) -> !` is more general than `fun<X>(X) -> X`.
  - (0:30-0:47): `fun<Y>(Y) -> !`
  - (0:10-0:24): `fun<X>(X) -> X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>, B, A)
```

### Output
```
<A: fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>
```

### Errors
- (0:50-0:51) Test failed because `fun<Y>(Y) -> !` is more general than `fun<Z>(Z) -> Z`.
  - (0:30-0:47): `fun<Y>(Y) -> !`
  - (0:10-0:24): `fun<Z>(Z) -> Z`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>
```

### Errors
- (0:51-0:52) Test failed because `fun<Y>(Y) -> !` is more general than `fun<X>(X) -> X`.
  - (0:31-0:48): `fun<Y>(Y) -> !`
  - (0:11-0:25): `fun<X>(X) -> X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>, B, A)
```

### Output
```
<A = fun<X>(X) -> X, B = fun<Y, Z>(Y) -> Z>
```

### Errors
- (0:51-0:52) Test failed because `fun<Y>(Y) -> !` is more general than `fun<Z>(Z) -> Z`.
  - (0:31-0:48): `fun<Y>(Y) -> !`
  - (0:31-0:48): `fun<Z>(Z) -> Z`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun(Int) -> Int>, A, B)
```

### Output
```
<A = fun(Int) -> Int, B = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun(Int) -> Int>, B, A)
```

### Output
```
<A = fun(Int) -> Int, B = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun(A) -> A>, A, B)
```

### Output
```
<A: !, B = fun(A) -> A>
```

### Errors
- (0:31-0:32) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun(A) -> A>, B, A)
```

### Output
```
<A: !, B = fun(A) -> A>
```

### Errors
- (0:31-0:32) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun<C>(C) -> A>, A, B)
```

### Output
```
<A: !, B = fun<C>(C) -> A>
```

### Errors
- (0:34-0:35) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun<C>(C) -> A>, B, A)
```

### Output
```
<A: !, B = fun<C>(C) -> A>
```

### Errors
- (0:34-0:35) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = !>, A, B)
```

### Output
```
<A = !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = !>, B, A)
```

### Output
```
<B = !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = fun<C>(C) -> C>, A, B)
```

### Output
```
<A = !, B = fun<C>(C) -> C>
```

### Errors
- (0:35-0:36) Test failed because `!` is more general than `fun<C>(C) -> C`.
  - (0:11-0:12): `!`
  - (0:18-0:32): `fun<C>(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = fun<C>(C) -> C>, B, A)
```

### Output
```
<A = !, B = fun<C>(C) -> C>
```

### Errors
- (0:35-0:36) Test failed because `!` is more general than `fun<C>(C) -> C`.
  - (0:11-0:12): `!`
  - (0:18-0:32): `fun<C>(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, B, A)
```

### Output
```
<B: !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, B, A)
```

### Output
```
<B: !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B: !>, A, B)
```

### Output
```
<A = !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B: !>, B, A)
```

### Output
```
<B = !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = !>, A, B)
```

### Output
```
<A = !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = !>, B, A)
```

### Output
```
<B = !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = !>, A, B)
```

### Output
```
<A = !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B = !>, B, A)
```

### Output
```
<B = !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B: fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<X>(X) -> X>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: fun<X>(X) -> X>, B, A)
```

### Output
```
<B: fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B: fun<X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B = fun<X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !>, X, X)
```

### Output
```
<X: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, A, Bool)
```

### Output
```
<A: Int>
```

### Errors
- (0:16-0:17) Test failed because `Int` is not a `Bool`.
  - (0:10-0:13): `Int`
  - (0:19-0:23): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, A, Bool)
```

### Output
```
<A = Int>
```

### Errors
- (0:17-0:18) Test failed because `Int` is not a `Bool`.
  - (0:11-0:14): `Int`
  - (0:20-0:24): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int>, Bool, A)
```

### Output
```
<A: Int>
```

### Errors
- (0:16-0:20) Test failed because `Bool` is not an `Int`.
  - (0:10-0:13): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int>, Bool, A)
```

### Output
```
<A = Int>
```

### Errors
- (0:17-0:21) Test failed because `Bool` is not an `Int`.
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = A>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = A>, B, A)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: A>, A, B)
```

### Output
```
<A: !, B: A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: A>, B, A)
```

### Output
```
<A: !, B: A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:51-0:52) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:44-0:48): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:51-0:52) Test failed because `Bool` is not an `Int`.
  - (0:44-0:48): `Bool`
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Bool` is not an `Int`.
  - (0:45-0:49): `Bool`
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Bool` is not an `Int`.
  - (0:45-0:49): `Bool`
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:53-0:54) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:46-0:50): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:53-0:54) Test failed because `Bool` is not an `Int`.
  - (0:46-0:50): `Bool`
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B: fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A: fun<X>(X) -> C, B: fun<Y>(Y) -> A>, A, B)
```

### Output
```
<C: !, A: fun<X>(X) -> C, B: fun<Y>(Y) -> A>
```

### Errors
- (0:52-0:53) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A: fun<X>(X) -> C, B: fun<Y>(Y) -> A>, B, A)
```

### Output
```
<C: !, A: fun<X>(X) -> C, B: fun<Y>(Y) -> A>
```

### Errors
- (0:52-0:53) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A = fun<X>(X) -> C, B: fun<Y>(Y) -> A>, A, B)
```

### Output
```
<C: !, A = fun<X>(X) -> C, B: fun<Y>(Y) -> A>
```

### Errors
- (0:53-0:54) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A = fun<X>(X) -> C, B: fun<Y>(Y) -> A>, B, A)
```

### Output
```
<C: !, A = fun<X>(X) -> C, B: fun<Y>(Y) -> A>
```

### Errors
- (0:53-0:54) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A: fun<X>(X) -> C, B = fun<Y>(Y) -> A>, A, B)
```

### Output
```
<C: !, A: fun<X>(X) -> C, B = fun<Y>(Y) -> A>
```

### Errors
- (0:53-0:54) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A: fun<X>(X) -> C, B = fun<Y>(Y) -> A>, B, A)
```

### Output
```
<C: !, A: fun<X>(X) -> C, B = fun<Y>(Y) -> A>
```

### Errors
- (0:53-0:54) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A = fun<X>(X) -> C, B = fun<Y>(Y) -> A>, A, B)
```

### Output
```
<C: !, A = fun<X>(X) -> C, B = fun<Y>(Y) -> A>
```

### Errors
- (0:54-0:55) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C: !, A = fun<X>(X) -> C, B = fun<Y>(Y) -> A>, B, A)
```

### Output
```
<C: !, A = fun<X>(X) -> C, B = fun<Y>(Y) -> A>
```

### Errors
- (0:54-0:55) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A: fun(Int) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<B: fun(Int) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>
```

### Errors
- (0:49-0:50) Test failed because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:32-0:46): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Y>
```

### Errors
- (0:49-0:50) Test failed because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:32-0:46): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>
```

### Errors
- (0:49-0:50) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:32-0:46): `fun<Y>(Y) -> Y`
  - (0:10-0:26): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>
```

### Errors
- (0:49-0:50) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:32-0:46): `fun<Y>(Y) -> Y`
  - (0:10-0:26): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>
```

### Errors
- (0:50-0:51) Test failed because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:11-0:27): `fun(Int) -> Int`
- (0:50-0:51) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:33-0:47): `fun<Y>(Y) -> Y`
  - (0:11-0:27): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Y>
```

### Errors
- (0:50-0:51) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:33-0:47): `fun<Y>(Y) -> Y`
  - (0:33-0:47): `fun(Int) -> Int`
- (0:50-0:51) Test failed because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:33-0:47): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: !>, A, fun(B) -> Bool)
```

### Output
```
<A: fun<X>(X) -> Int, B: !>
```

### Errors
- (0:35-0:36) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:48-0:52): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: !>, A, fun(B) -> Int)
```

### Output
```
<B: !, A = fun(B) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: !>, A, fun(B) -> Bool)
```

### Output
```
<A = fun<X>(X) -> Int, B: !>
```

### Errors
- (0:36-0:37) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:49-0:53): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: !>, A, fun(B) -> Int)
```

### Output
```
<A = fun<X>(X) -> Int, B: !>
```

### Errors
- (0:36-0:37) Test failed because `fun<X>(X) -> Int` is more general than `fun(B) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:39-0:52): `fun(B) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, A, fun(Bool) -> Bool)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:29-0:30) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, A, fun(Bool) -> Int)
```

### Output
```
<A = fun(Bool) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, A, fun(Bool) -> Bool)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:31) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:46-0:50): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, A, fun(Bool) -> Int)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:31) Test failed because `fun<X>(X) -> Int` is more general than `fun(Bool) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`
  - (0:33-0:49): `fun(Bool) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, A, fun(A) -> Bool)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:29-0:30) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:42-0:46): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, A, fun(A) -> Int)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:29-0:30) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, A, fun(A) -> Bool)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:31) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:43-0:47): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, A, fun(A) -> Int)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:31) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: !>, fun(B) -> Bool, A)
```

### Output
```
<A: fun<X>(X) -> Int, B: !>
```

### Errors
- (0:45-0:49) Test failed because `Bool` is not an `Int`.
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: !>, fun(B) -> Int, A)
```

### Output
```
<B: !, A = fun(B) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: !>, fun(B) -> Bool, A)
```

### Output
```
<A = fun<X>(X) -> Int, B: !>
```

### Errors
- (0:46-0:50) Test failed because `Bool` is not an `Int`.
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: !>, fun(B) -> Int, A)
```

### Output
```
<A = fun<X>(X) -> Int, B: !>
```

### Errors
- (0:36-0:49) Test failed because `fun<X>(X) -> Int` is more general than `fun(B) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, fun(Bool) -> Bool, A)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:42-0:46) Test failed because `Bool` is not an `Int`.
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, fun(Bool) -> Int, A)
```

### Output
```
<A = fun(Bool) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, fun(Bool) -> Bool, A)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:43-0:47) Test failed because `Bool` is not an `Int`.
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, fun(Bool) -> Int, A)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:46) Test failed because `fun<X>(X) -> Int` is more general than `fun(Bool) -> Int`.
  - (0:11-0:27): `fun<X>(X) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, fun(A) -> Bool, A)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:39-0:43) Test failed because `Bool` is not an `Int`.
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int>, fun(A) -> Int, A)
```

### Output
```
<A: fun<X>(X) -> Int>
```

### Errors
- (0:29-0:42) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, fun(A) -> Bool, A)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:40-0:44) Test failed because `Bool` is not an `Int`.
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int>, fun(A) -> Int, A)
```

### Output
```
<A = fun<X>(X) -> Int>
```

### Errors
- (0:30-0:43) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun<X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = fun<X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: fun<X>(X) -> X>, A, B)
```

### Output
```
<A: fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: fun<X>(X) -> X>, B, A)
```

### Output
```
<B: fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:51-0:52) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:44-0:48): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:51-0:52) Test failed because `Bool` is not an `Int`.
  - (0:44-0:48): `Bool`
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B: fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Bool` is not an `Int`.
  - (0:45-0:49): `Bool`
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B: fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Int` is not a `Bool`.
  - (0:23-0:26): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:52-0:53) Test failed because `Bool` is not an `Int`.
  - (0:45-0:49): `Bool`
  - (0:23-0:26): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:53-0:54) Test failed because `Int` is not a `Bool`.
  - (0:24-0:27): `Int`
  - (0:46-0:50): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>, B, A)
```

### Output
```
<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Bool>
```

### Errors
- (0:53-0:54) Test failed because `Bool` is not an `Int`.
  - (0:46-0:50): `Bool`
  - (0:24-0:27): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<X>(X) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = A, C = B, D = C, E = D, F = fun(E) -> E>, A, F)
```

### Output
```
<A: !, B = A, C = B, D = C, E = D, F = fun(E) -> E>
```

### Errors
- (0:59-0:60) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B = A, C = B, D = C, E = D, F = fun(E) -> E>, F, A)
```

### Output
```
<A: !, B = A, C = B, D = C, E = D, F = fun(E) -> E>
```

### Errors
- (0:59-0:60) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, fun(A) -> A, fun(A) -> B)
```

### Output
```
<A = fun<X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X>(X) -> X, B = fun<Y>(Y) -> Y>, fun(A) -> B, fun(A) -> A)
```

### Output
```
<B = fun<Y>(Y) -> Y, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = fun<A = fun<X>(X) -> X>(A) -> A, U = <B = fun<Y>(Y) -> Y, C = fun<Z>(Z) -> Z> fun(B) -> C>, T, U)
```

### Output
```
<T = fun<A = fun<X>(X) -> X>(A) -> A, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = fun<A = fun<X>(X) -> X>(A) -> A, U = <B = fun<Y>(Y) -> Y, C = fun<Z>(Z) -> Z> fun(B) -> C>, U, T)
```

### Output
```
<U = fun<C = fun<Z>(Z) -> Z>(C) -> C, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !>, A, B)
```

### Output
```
<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !>, B, A)
```

### Output
```
<B = fun<X, X = fun<Z>(Z) -> X>(X) -> X, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !, C: !, D: !, E: !>, A, fun(fun(B) -> C) -> fun(D) -> E)
```

### Output
```
<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !, C: !, D: !, E = C>
```

### Errors
- (0:72-0:73) Test failed because `fun<Z>(Z) -> X` is more general than `fun(B) -> C`.
  - (0:22-0:36): `fun<Z>(Z) -> X`
  - (0:79-0:90): `fun(B) -> C`
- (0:72-0:73) Test failed because `fun<Z>(Z) -> X` is more general than `fun(D) -> E`.
  - (0:22-0:36): `fun<Z>(Z) -> X`
  - (0:95-0:106): `fun(D) -> E`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !, C: !, D: !, E: !>, fun(fun(B) -> C) -> fun(D) -> E, A)
```

### Output
```
<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B: !, E: !, C = E, D: !>
```

### Errors
- (0:76-0:87) Test failed because `fun<Z>(Z) -> X` is more general than `fun(B) -> C`.
  - (0:22-0:36): `fun<Z>(Z) -> X`
- (0:92-0:103) Test failed because `fun<Z>(Z) -> X` is more general than `fun(D) -> E`.
  - (0:22-0:36): `fun<Z>(Z) -> X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X, X: fun<Z>(Z) -> X>(X) -> X, B: !, C: !, D: !, E: !>, A, fun(fun(B) -> C) -> fun(D) -> E)
```

### Output
```
<B: !, C: !, D = B, E = C, A = fun(fun(B) -> C) -> fun(D) -> E>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X, X: fun<Z>(Z) -> X>(X) -> X, B: !, C: !, D: !, E: !>, fun(fun(B) -> C) -> fun(D) -> E, A)
```

### Output
```
<D: !, B = D, E: !, C = E, A = fun(fun(B) -> C) -> fun(D) -> E>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B = fun<X, X = fun<Z>(Z) -> X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X, X2 = fun<Z>(Z) -> X>(X2) -> X2, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X2, X = fun<Z>(Z) -> fun(X2) -> X>(X) -> X, B = fun<X, X2, X = fun<Z>(Z) -> fun(X2) -> X>(X) -> X>, A, B)
```

### Output
```
<A = fun<X, X2, X3 = fun<Z>(Z) -> fun(X2) -> X>(X3) -> X3, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X1, X1 = fun<Z>(Z) -> X1>(X1) -> X1, B = fun<X1, X1 = fun<Z>(Z) -> X1>(X1) -> X1>, A, B)
```

### Output
```
<A = fun<X1, X2 = fun<Z>(Z) -> X1>(X2) -> X2, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X1, X2, X1 = fun<Z>(Z) -> fun(X2) -> X1>(X1) -> X1, B = fun<X1, X2, X1 = fun<Z>(Z) -> fun(X2) -> X1>(X1) -> X1>, A, B)
```

### Output
```
<A = fun<X1, X2, X3 = fun<Z>(Z) -> fun(X2) -> X1>(X3) -> X3, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X5, X5 = fun<Z>(Z) -> X5>(X5) -> X5, B = fun<X5, X5 = fun<Z>(Z) -> X5>(X5) -> X5>, A, B)
```

### Output
```
<A = fun<X5, X6 = fun<Z>(Z) -> X5>(X6) -> X6, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X = fun<Z>(Z) -> X>(X) -> X, B = fun<X, X = fun<Z>(Z) -> X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X, X2 = fun<Z>(Z) -> X>(X2) -> X2, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, X2, X = fun<Z>(Z) -> fun(X2) -> X>(X) -> X, B = fun<X, X2, X = fun<Z>(Z) -> fun(X2) -> X>(X) -> X>, B, A)
```

### Output
```
<B = fun<X, X2, X3 = fun<Z>(Z) -> fun(X2) -> X>(X3) -> X3, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X1, X1 = fun<Z>(Z) -> X1>(X1) -> X1, B = fun<X1, X1 = fun<Z>(Z) -> X1>(X1) -> X1>, B, A)
```

### Output
```
<B = fun<X1, X2 = fun<Z>(Z) -> X1>(X2) -> X2, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X1, X2, X1 = fun<Z>(Z) -> fun(X2) -> X1>(X1) -> X1, B = fun<X1, X2, X1 = fun<Z>(Z) -> fun(X2) -> X1>(X1) -> X1>, B, A)
```

### Output
```
<B = fun<X1, X2, X3 = fun<Z>(Z) -> fun(X2) -> X1>(X3) -> X3, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X5, X5 = fun<Z>(Z) -> X5>(X5) -> X5, B = fun<X5, X5 = fun<Z>(Z) -> X5>(X5) -> X5>, B, A)
```

### Output
```
<B = fun<X5, X6 = fun<Z>(Z) -> X5>(X6) -> X6, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<X>(X) -> X, B: !>, A, fun(Int) -> B)
```

### Output
```
<B = Int, A = fun(Int) -> B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <X = Int> X>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X: fun<Y>(Y) -> Y> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <X: fun<Y>(Y) -> Y> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun<Y>(Y) -> Y>
```

### Errors
- (0:35-0:36) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:15-0:29): `fun<Y>(Y) -> Y`
  - (0:38-0:53): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X = fun<Y>(Y) -> Y> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <X = fun<Y>(Y) -> Y> X>, A, fun(Int) -> Int)
```

### Output
```
<A = fun<Y>(Y) -> Y>
```

### Errors
- (0:36-0:37) Test failed because `fun<Y>(Y) -> Y` is more general than `fun(Int) -> Int`.
  - (0:16-0:30): `fun<Y>(Y) -> Y`
  - (0:39-0:54): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<Y>(Y) -> Y>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = fun<X>(X) -> X> B>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = fun<X>(X) -> X> C> B>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = <D = fun<X>(X) -> X> D> C> B>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = fun<X>(X) -> X> fun(B) -> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<B = fun<X>(X) -> X>(B) -> B>
```

### Errors
- (0:45-0:46) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:15-0:29): `fun<X>(X) -> X`
  - (0:52-0:67): `fun(Int) -> Int`
- (0:45-0:46) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:15-0:29): `fun<X>(X) -> X`
  - (0:72-0:87): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = fun<X>(X) -> X> C> fun(B) -> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<B = fun<X>(X) -> X>(B) -> B>
```

### Errors
- (0:53-0:54) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:20-0:34): `fun<X>(X) -> X`
  - (0:60-0:75): `fun(Int) -> Int`
- (0:53-0:54) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:20-0:34): `fun<X>(X) -> X`
  - (0:80-0:95): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = <D = fun<X>(X) -> X> D> C> fun(B) -> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<B = fun<X>(X) -> X>(B) -> B>
```

### Errors
- (0:61-0:62) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:25-0:39): `fun<X>(X) -> X`
  - (0:68-0:83): `fun(Int) -> Int`
- (0:61-0:62) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:25-0:39): `fun<X>(X) -> X`
  - (0:88-0:103): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = fun<X>(X) -> X> fun(B) -> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<B = fun<X>(X) -> X>(B) -> B>
```

### Errors
- (0:45-0:46) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:15-0:29): `fun<X>(X) -> X`
  - (0:52-0:67): `fun(Int) -> Int`
- (0:45-0:46) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:15-0:29): `fun<X>(X) -> X`
  - (0:72-0:87): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = fun<X>(X) -> X> fun(C) -> C> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<C = fun<X>(X) -> X>(C) -> C>
```

### Errors
- (0:53-0:54) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:20-0:34): `fun<X>(X) -> X`
  - (0:60-0:75): `fun(Int) -> Int`
- (0:53-0:54) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:20-0:34): `fun<X>(X) -> X`
  - (0:80-0:95): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <B = <C = <D = fun<X>(X) -> X> fun(D) -> D> C> B>, A, fun(fun(Int) -> Int) -> fun(Int) -> Int)
```

### Output
```
<A: fun<D = fun<X>(X) -> X>(D) -> D>
```

### Errors
- (0:61-0:62) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:25-0:39): `fun<X>(X) -> X`
  - (0:68-0:83): `fun(Int) -> Int`
- (0:61-0:62) Test failed because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`.
  - (0:25-0:39): `fun<X>(X) -> X`
  - (0:88-0:103): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X: !> X>, A, Int)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X: !> X>, Int, A)
```

### Output
```
<A = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X: !> X, B: <X: !> X>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X: !> X, B: <X: !> X>, B, A)
```

### Output
```
<B: !, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, Y>(Int) -> Int>, A, fun(Int) -> Int)
```

### Output
```
<A = fun(Int) -> Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, Y>(Y) -> Int, B = fun<X, Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<Y>(Y) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, Y>(Y) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<Y>(Y) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<X, Y>(Y) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<Y, X>(Y) -> Int, B = fun<Y, X>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<Y>(Y) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<Y, X>(Y) -> Int, B = fun<Y>(Y) -> Int>, A, B)
```

### Output
```
<A = fun<Y>(Y) -> Int, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<Y, X>(Y) -> Int, B = fun<Y>(Y) -> Int>, B, A)
```

### Output
```
<B = fun<Y>(Y) -> Int, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: fun<B>(B) -> B> fun(A) -> A, Y = fun<C>(fun(C) -> C) -> (fun(C) -> C)>, X, Y)
```

### Output
```
<X = fun<A: fun<B>(B) -> B>(A) -> A, Y = fun<C>(fun(C) -> C) -> fun(C) -> C>
```

### Errors
- (0:87-0:88) Test failed because `fun<A: fun<B>(B) -> B>(A) -> A` is more general than `fun<C>(fun(C) -> C) -> fun(C) -> C`.
  - (0:31-0:42): `fun<A: fun<B>(B) -> B>(A) -> A`
  - (0:31-0:42): `fun<C>(fun(C) -> C) -> fun(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: fun<B>(B) -> B> fun(A) -> A, Y = fun<C>(fun(C) -> C) -> (fun(C) -> C)>, X, Y)
```

### Output
```
<X = fun<C>(fun(C) -> C) -> fun(C) -> C, Y = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: fun<B>(B) -> B> fun(A) -> A, Y: fun<C>(fun(C) -> C) -> (fun(C) -> C)>, X, Y)
```

### Output
```
<X = fun<A: fun<B>(B) -> B>(A) -> A, Y: fun<C>(fun(C) -> C) -> fun(C) -> C>
```

### Errors
- (0:86-0:87) Test failed because `fun<A: fun<B>(B) -> B>(A) -> A` is more general than `fun<C>(fun(C) -> C) -> fun(C) -> C`.
  - (0:31-0:42): `fun<A: fun<B>(B) -> B>(A) -> A`
  - (0:47-0:83): `fun<C>(fun(C) -> C) -> fun(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: fun<B>(B) -> B> fun(A) -> A, Y: fun<C>(fun(C) -> C) -> (fun(C) -> C)>, X, Y)
```

### Output
```
<X: fun<C>(fun(C) -> C) -> fun(C) -> C, Y = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: fun<B>(B) -> B> fun(A) -> A, Y = fun<C>(fun(C) -> C) -> (fun(C) -> C)>, Y, X)
```

### Output
```
<X = fun<A: fun<B>(B) -> B>(A) -> A, Y = fun<C>(fun(C) -> C) -> fun(C) -> C>
```

### Errors
- (0:87-0:88) Test failed because `fun<A: fun<B>(B) -> B>(A) -> A` is more general than `fun<C>(fun(C) -> C) -> fun(C) -> C`.
  - (0:31-0:42): `fun<A: fun<B>(B) -> B>(A) -> A`
  - (0:48-0:84): `fun<C>(fun(C) -> C) -> fun(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: fun<B>(B) -> B> fun(A) -> A, Y = fun<C>(fun(C) -> C) -> (fun(C) -> C)>, Y, X)
```

### Output
```
<Y = fun<C>(fun(C) -> C) -> fun(C) -> C, X = Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: fun<B>(B) -> B> fun(A) -> A, Y: fun<C>(fun(C) -> C) -> (fun(C) -> C)>, Y, X)
```

### Output
```
<X = fun<A: fun<B>(B) -> B>(A) -> A, Y: fun<C>(fun(C) -> C) -> fun(C) -> C>
```

### Errors
- (0:86-0:87) Test failed because `fun<A: fun<B>(B) -> B>(A) -> A` is more general than `fun<C>(fun(C) -> C) -> fun(C) -> C`.
  - (0:31-0:42): `fun<A: fun<B>(B) -> B>(A) -> A`
  - (0:47-0:83): `fun<C>(fun(C) -> C) -> fun(C) -> C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: fun<B>(B) -> B> fun(A) -> A, Y: fun<C>(fun(C) -> C) -> (fun(C) -> C)>, Y, X)
```

### Output
```
<Y: fun<C>(fun(C) -> C) -> fun(C) -> C, X = Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = nope>, T, T)
```

### Output
```
<T = !>
```

### Errors
- (0:11-0:15) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T1 = nope, T2 = nope>, T1, T2)
```

### Output
```
<T1 = !, T2 = T1>
```

### Errors
- (0:12-0:16) Can not find `nope`.
- (0:23-0:27) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T1 = nope1, T2 = nope2>, T1, T2)
```

### Output
```
<T1 = !, T2 = T1>
```

### Errors
- (0:12-0:17) Can not find `nope1`.
- (0:24-0:29) Can not find `nope2`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y = Int>, X, Y)
```

### Output
```
<X = Int, Y = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y = Int>, Y, X)
```

### Output
```
<X = Int, Y = Int>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y = nope>, X, Y)
```

### Output
```
<X = !, Y = X>
```

### Errors
- (0:17-0:21) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y = nope>, Y, X)
```

### Output
```
<Y = !, X = Y>
```

### Errors
- (0:17-0:21) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<Z>(Z) -> nope, B = fun<Z>(Z) -> nope>, A, B)
```

### Output
```
<A = fun<Type1, Z>(Z) -> Type1, B = A>
```

### Errors
- (0:24-0:28) Can not find `nope`.
- (0:47-0:51) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<Z>(Z) -> nope, B = fun<Z>(Z) -> nope>, B, A)
```

### Output
```
<B = fun<Type1, Z>(Z) -> Type1, A = B>
```

### Errors
- (0:24-0:28) Can not find `nope`.
- (0:47-0:51) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C = nope, A = fun<Z>(Z) -> C, B = fun<Z>(Z) -> C>, A, B)
```

### Output
```
<C = !, A = fun<Z>(Z) -> C, B = A>
```

### Errors
- (0:11-0:15) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<C = nope, A = fun<Z>(Z) -> C, B = fun<Z>(Z) -> C>, B, A)
```

### Output
```
<C = !, B = fun<Z>(Z) -> C, A = B>
```

### Errors
- (0:11-0:15) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E = Int, A = fun<Z>(Z) -> E, B = fun<Z>(Z) -> E> fun(A) -> B>, T, fun(X) -> Y)
```

### Output
```
<X = fun<Z>(Z) -> Int, Y = fun<Z>(Z) -> Int, T = fun(X) -> Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E = Int, A = fun<Z>(Z) -> E, B = fun<Z>(Z) -> E> fun(A) -> B>, fun(X) -> Y, T)
```

### Output
```
<X = fun<Z>(Z) -> Int, Y = fun<Z>(Z) -> Int, T = fun(X) -> Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E = nope, A = fun<Z>(Z) -> E, B = fun<Z>(Z) -> E> fun(A) -> B>, T, fun(X) -> Y)
```

### Output
```
<E = !, X = fun<Z>(Z) -> E, Y = fun<Z>(Z) -> E, T = fun(X) -> Y>
```

### Errors
- (0:28-0:32) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E = nope, A = fun<Z>(Z) -> E, B = fun<Z>(Z) -> E> fun(A) -> B>, fun(X) -> Y, T)
```

### Output
```
<E = !, X = fun<Z>(Z) -> E, Y = fun<Z>(Z) -> E, T = fun(X) -> Y>
```

### Errors
- (0:28-0:32) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E1 = nope, E2 = nope, A = fun<Z>(Z) -> E1, B = fun<Z>(Z) -> E2> fun(A) -> B>, T, fun(X) -> Y)
```

### Output
```
<E1 = !, E2 = !, X = fun<Z>(Z) -> E1, Y = fun<Z>(Z) -> E2, T = fun(X) -> Y>
```

### Errors
- (0:29-0:33) Can not find `nope`.
- (0:40-0:44) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E1 = nope, E2 = nope, A = fun<Z>(Z) -> E1, B = fun<Z>(Z) -> E2> fun(A) -> B>, fun(X) -> Y, T)
```

### Output
```
<E1 = !, E2 = !, X = fun<Z>(Z) -> E1, Y = fun<Z>(Z) -> E2, T = fun(X) -> Y>
```

### Errors
- (0:29-0:33) Can not find `nope`.
- (0:40-0:44) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <A = fun<Z>(Z) -> nope, B = fun<Z>(Z) -> nope> fun(A) -> B>, T, fun(X) -> Y)
```

### Output
```
<X = fun<Z, Type1>(Z) -> Type1, Y = fun<Z, Type1>(Z) -> Type1, T = fun(X) -> Y>
```

### Errors
- (0:41-0:45) Can not find `nope`.
- (0:64-0:68) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <A = fun<Z>(Z) -> nope, B = fun<Z>(Z) -> nope> fun(A) -> B>, fun(X) -> Y, T)
```

### Output
```
<X = fun<Z, Type1>(Z) -> Type1, Y = fun<Z, Type1>(Z) -> Type1, T = fun(X) -> Y>
```

### Errors
- (0:41-0:45) Can not find `nope`.
- (0:64-0:68) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E = nope, A = fun<Z>(Z) -> E, B = fun<Z>(Z) -> E> fun(A) -> B>, fun(T) -> (fun(Y) -> X), fun(fun(X) -> Y) -> (fun(X) -> Y))
```

### Output
```
<E = !, Y = fun<Z>(Z) -> E, X = Y, T = fun(X) -> Y>
```

### Errors
- (0:28-0:32) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <E1 = nope, E2 = nope, A = fun<Z>(Z) -> E1, B = fun<Z>(Z) -> E2> fun(A) -> B>, fun(T) -> (fun(Y) -> X), fun(fun(X) -> Y) -> (fun(X) -> Y))
```

### Output
```
<E2 = !, E1 = E2, Y = fun<Z>(Z) -> E2, X = Y, T = fun(X) -> Y>
```

### Errors
- (0:29-0:33) Can not find `nope`.
- (0:40-0:44) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !, Y: !, T = <A = fun<Z>(Z) -> nope, B = fun<Z>(Z) -> nope> fun(A) -> B>, fun(T) -> (fun(Y) -> X), fun(fun(X) -> Y) -> (fun(X) -> Y))
```

### Output
```
<Y = fun<Type1, Z>(Z) -> Type1, X = Y, T = fun(X) -> Y>
```

### Errors
- (0:41-0:45) Can not find `nope`.
- (0:64-0:68) Can not find `nope`.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !>, A, B)
```

### Output
```
<A: !, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, void, void)
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, Bool, void)
```

### Output
```
<>
```

### Errors
- (0:10-0:14) Test failed because `Bool` is not void.
  - (0:16-0:20): void

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, void, Bool)
```

### Output
```
<>
```

### Errors
- (0:10-0:14) Test failed because void is not a `Bool`.
  - (0:16-0:20): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Int, b: Int, c: void})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:50-0:53): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Bool, b: Bool, c: void})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:42-0:46): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Int, b: Bool, c: Bool})
```

### Output
```
<>
```

### Errors
- (0:31-0:35) Test failed because void is not a `Bool`.
  - (0:59-0:63): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Int, b: Bool, c: void})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Bool, b: Int, c: void})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:42-0:46): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:51-0:54): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: Int, b: void, c: Bool})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not void.
  - (0:50-0:54): void
- (0:31-0:35) Test failed because void is not a `Bool`.
  - (0:59-0:63): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool, c: void}, {a: void, b: Bool, c: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not void.
  - (0:42-0:46): void
- (0:31-0:35) Test failed because void is not an `Int`.
  - (0:60-0:63): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool}, {a: Bool, b: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:33-0:37): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:42-0:45): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Bool, a: Int}, {b: Int, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:23-0:26) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`
- (0:14-0:18) Test failed because `Bool` is not an `Int`.
  - (0:33-0:36): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool}, {b: Int, b: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `b:` is missing.
  - (0:30-0:31): `b:`
- (0:10-0:27) Test failed because `b:` is missing.
  - (0:38-0:39): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.
- (0:19-0:20) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {})
```

### Output
```
<>
```

### Errors
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:12) Test failed because `a:` is missing.
  - (0:15-0:16): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int | Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:10-0:18) Test failed because an object is not an `Int`.
  - (0:30-0:33): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int | {}}, {b: Int})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `b:` is missing.
  - (0:26-0:27): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int | {a: Int}})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Int | {a: Int}}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:11-0:12) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {}}, {})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {}, {| {}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {}}, {| {}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {| {a: Int}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {| {a: Bool}})
```

### Output
```
<>
```

### Errors
- (0:17-0:20) Test failed because `Int` is not a `Bool`.
  - (0:31-0:35): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {| {b: Int}})
```

### Output
```
<>
```

### Errors
- (0:13-0:21) Test failed because `b:` is missing.
  - (0:28-0:29): `b:`
- (0:14-0:15) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {| {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:15) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {}}, {| {a: Int}})
```

### Output
```
<>
```

### Errors
- (0:13-0:15) Test failed because `a:` is missing.
  - (0:22-0:23): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {a: Int})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {| {a: Int}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {a: Bool})
```

### Output
```
<>
```

### Errors
- (0:17-0:20) Test failed because `Int` is not a `Bool`.
  - (0:28-0:32): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Bool}, {| {a: Int}})
```

### Output
```
<>
```

### Errors
- (0:14-0:18) Test failed because `Bool` is not an `Int`.
  - (0:28-0:31): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {| {a: Int}}, {b: Bool})
```

### Output
```
<>
```

### Errors
- (0:13-0:21) Test failed because `b:` is missing.
  - (0:25-0:26): `b:`
- (0:14-0:15) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Bool}, {| {a: Int}})
```

### Output
```
<>
```

### Errors
- (0:10-0:19) Test failed because `a:` is missing.
  - (0:25-0:26): `a:`
- (0:11-0:12) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:12) Test failed because `a:` is missing.
  - (0:15-0:16): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Int}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `a:` is missing.
  - (0:21-0:22): `a:`
- (0:11-0:12) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Int | {}}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `a:` is missing.
  - (0:26-0:27): `a:`
- (0:11-0:12) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Int, a: Int, a: void})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:50-0:53): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Bool, a: Bool, a: void})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:42-0:46): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Int, a: Bool, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:31-0:35) Test failed because void is not a `Bool`.
  - (0:59-0:63): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Int, a: Bool, a: void})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Bool, a: Int, a: void})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:42-0:46): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:51-0:54): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: Int, a: void, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not void.
  - (0:50-0:54): void
- (0:31-0:35) Test failed because void is not a `Bool`.
  - (0:59-0:63): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, a: void}, {a: void, a: Bool, a: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not void.
  - (0:42-0:46): void
- (0:31-0:35) Test failed because void is not an `Int`.
  - (0:60-0:63): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, b: Bool})
```

### Output
```
<T = {b: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, b: Bool}, {a: Int | T})
```

### Output
```
<T = {b: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Bool, b: Int})
```

### Output
```
<T = {b: Int}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Bool, b: Int}, {a: Int | T})
```

### Output
```
<T = {b: Int}>
```

### Errors
- (0:18-0:22) Test failed because `Bool` is not an `Int`.
  - (0:37-0:40): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, b: Bool, b: Int})
```

### Output
```
<T = {b: Bool, b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, b: Bool, b: Int}, {a: Int | T})
```

### Output
```
<T = {b: Bool, b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, a: Bool})
```

### Output
```
<T = {a: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, a: Bool}, {a: Int | T})
```

### Output
```
<T = {a: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Bool, a: Int})
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Bool, a: Int}, {a: Int | T})
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:18-0:22) Test failed because `Bool` is not an `Int`.
  - (0:37-0:40): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, a: Bool, a: void})
```

### Output
```
<T = {a: Bool, a: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, a: Bool, a: void}, {a: Int | T})
```

### Output
```
<T = {a: Bool, a: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int}) -> {a: Int}, fun({a: Int | T}) -> T)
```

### Output
```
<T = {}>
```

### Errors
- (0:32-0:33) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int, a: Int}) -> {a: Int}, fun({a: Int | T}) -> T)
```

### Output
```
<T = {a: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int}) -> {a: Int}, fun(T) -> {a: Int | T})
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:31-0:39) Test failed because `a:` is missing.
  - (0:19-0:20): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int}) -> Int, fun({a: Int | T}) -> T)
```

### Output
```
<T = {}>
```

### Errors
- (0:31-0:34) Test failed because `Int` is not an object.
  - (0:18-0:26): object

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int}) -> Int, fun({a: Int, a: Int | T}) -> T)
```

### Output
```
<T = Int>
```

### Errors
- (0:18-0:26) Test failed because `a:` is missing.
  - (0:49-0:50): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int}) -> Int, fun(T) -> {a: Int | T})
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:31-0:34) Test failed because `Int` is not an object.
  - (0:46-0:58): object

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int | T}) -> T, fun({a: Int}) -> {a: Int})
```

### Output
```
<T = {}>
```

### Errors
- (0:35-0:36) Test failed because `a:` is missing.
  - (0:56-0:57): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun(T) -> {a: Int | T}, fun({a: Int}) -> {a: Int})
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:34-0:35) Test failed because `a:` is not needed.
  - (0:43-0:44): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun({a: Int | T}) -> T, fun({a: Int}) -> Int)
```

### Output
```
<T = {}>
```

### Errors
- (0:35-0:36) Test failed because an object is not an `Int`.
  - (0:42-0:50): object
  - (0:55-0:58): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, fun(T) -> {a: Int | T}, fun({a: Int}) -> Int)
```

### Output
```
<T = {a: Int}>
```

### Errors
- (0:24-0:36) Test failed because an object is not an `Int`.
  - (0:55-0:58): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, T)
```

### Output
```
<T: !>
```

### Errors
- (0:14-0:26) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, T, {a: Int | T})
```

### Output
```
<T: !>
```

### Errors
- (0:14-0:15) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {b: Int}, {a: Int | T})
```

### Output
```
<Type1: !, T = {b: Int | Type1}>
```

### Errors
- (0:14-0:22) Test failed because `a:` is missing.
  - (0:25-0:26): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {b: Int})
```

### Output
```
<Type1: !, T = {b: Int | Type1}>
```

### Errors
- (0:15-0:16) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, b: Int}, {a: Int | T})
```

### Output
```
<T = {b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, b: Int})
```

### Output
```
<T = {b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l1: A, l2: B}>, A, B)
```

### Output
```
<A: !, B = A, C = {l1: A, l2: B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l1: A, l2: B}>, B, A)
```

### Output
```
<B: !, A = B, C = {l1: A, l2: B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l: Int | A}, D = {l: Int | B}>, A, B)
```

### Output
```
<A: !, B = A, C = {l: Int | A}, D = {l: Int | B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l: Int | A}, D = {l: Int | B}>, B, A)
```

### Output
```
<B: !, A = B, C = {l: Int | A}, D = {l: Int | B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l: A | B}>, A, B)
```

### Output
```
<A: !, B = A, C = {l: A | B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, C = {l: A | B}>, B, A)
```

### Output
```
<B: !, A = B, C = {l: A | B}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, R: !, S: !>, {p: A | R}, {q: B | S})
```

### Output
```
<A: !, B: !, Type1: !, R = {q: B | Type1}, S = {p: A | Type1}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, R: !, S: !>, R, {p: A | S})
```

### Output
```
<A: !, S: !, R = {p: A | S}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, S: !, T: !, R = {p: A | S}>, R, {q: B | T})
```

### Output
```
<A: !, B: !, Type1: !, S = {q: B | Type1}, R = {p: A | S}, T = {p: A | Type1}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !, B: !, S: !, T: !, R = {p: A | S}>, {q: B | T}, R)
```

### Output
```
<A: !, B: !, Type1: !, S = {q: B | Type1}, R = {p: A | S}, T = {p: A | Type1}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<S: !, T: !, R = {p: Int | S}>, R, {p: Bool | T})
```

### Output
```
<S: !, R = {p: Int | S}, T = S>
```

### Errors
- (0:38-0:39) Test failed because `Int` is not a `Bool`.
  - (0:27-0:30): `Int`
  - (0:45-0:49): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<S: !, T: !, R = {p: Int | S}>, {p: Bool | T}, R)
```

### Output
```
<T: !, S = T, R = {p: Int | S}>
```

### Errors
- (0:42-0:46) Test failed because `Bool` is not an `Int`.
  - (0:27-0:30): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, {p: A})
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:15) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {p: A}, A)
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:20) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, A, {p: Int | A})
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:15) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {p: Int | A}, A)
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:26) Test failed because the type checker infers an infinite type.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {x: Int}, {y: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `y:` is missing.
  - (0:21-0:22): `y:`
- (0:11-0:12) Test failed because `x:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {x: Int | {}}, {y: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `y:` is missing.
  - (0:26-0:27): `y:`
- (0:11-0:12) Test failed because `x:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {x: Int | A}, {x: Int | A})
```

### Output
```
<A: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {x: Int | A}, {x: Int, y: Int | A})
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:26) Test failed because `y:` is missing.
  - (0:37-0:38): `y:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {x: Int, y: Int | A}, {x: Int | A})
```

### Output
```
<A: !>
```

### Errors
- (0:23-0:24) Test failed because `y:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: !>, {x: Int | A}, {y: Int | A})
```

### Output
```
<A: !>
```

### Errors
- (0:14-0:26) Test failed because `y:` is missing.
  - (0:29-0:30): `y:`
- (0:15-0:16) Test failed because `x:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, X: !>, X, {p: Bool | R})
```

### Output
```
<R: !, X = {p: Bool | R}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, X = {p: Bool | R}>, X, {q: Int | R})
```

### Output
```
<R: !, X = {p: Bool | R}>
```

### Errors
- (0:33-0:34) Test failed because `q:` is missing.
  - (0:37-0:38): `q:`
- (0:33-0:34) Test failed because `p:` is not needed.
  - (0:18-0:19): `p:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {}, {})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Int})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:24-0:28): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool}, {b: Bool, a: Int})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: Bool}, {b: Int, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:33-0:36): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int | {b: Bool}}, {b: Bool | {a: Int}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int | {b: Bool}}, {b: Int | {a: Bool}})
```

### Output
```
<>
```

### Errors
- (0:24-0:28) Test failed because `Bool` is not an `Int`.
  - (0:36-0:39): `Int`
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:46-0:50): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Bool}, Y = {a: Int}>, {a: Int | X}, {b: Bool | Y})
```

### Output
```
<X = {b: Bool}, Y = {a: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Bool}, Y = {a: Bool}>, {a: Int | X}, {b: Int | Y})
```

### Output
```
<X = {b: Bool}, Y = {a: Bool}>
```

### Errors
- (0:48-0:49) Test failed because `Bool` is not an `Int`.
  - (0:15-0:19): `Bool`
  - (0:56-0:59): `Int`
- (0:42-0:45) Test failed because `Int` is not a `Bool`.
  - (0:30-0:34): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Bool}>, {a: Int | X}, {b: Bool | {a: Int}})
```

### Output
```
<X = {b: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Bool}>, {a: Int | X}, {b: Int | {a: Bool}})
```

### Output
```
<X = {b: Bool}>
```

### Errors
- (0:33-0:34) Test failed because `Bool` is not an `Int`.
  - (0:15-0:19): `Bool`
  - (0:41-0:44): `Int`
- (0:27-0:30) Test failed because `Int` is not a `Bool`.
  - (0:51-0:55): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Bool}>, {a: Int | X}, {b: Int | {a: Int}})
```

### Output
```
<X = {b: Bool}>
```

### Errors
- (0:33-0:34) Test failed because `Bool` is not an `Int`.
  - (0:15-0:19): `Bool`
  - (0:41-0:44): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = {b: Int}>, {a: Int | X}, {b: Int | {a: Bool}})
```

### Output
```
<X = {b: Int}>
```

### Errors
- (0:26-0:29) Test failed because `Int` is not a `Bool`.
  - (0:50-0:54): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<Y = {a: Int}>, {a: Int | {b: Bool}}, {b: Bool | Y})
```

### Output
```
<Y = {a: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<Y = {a: Bool}>, {a: Int | {b: Bool}}, {b: Int | Y})
```

### Output
```
<Y = {a: Bool}>
```

### Errors
- (0:37-0:41) Test failed because `Bool` is not an `Int`.
  - (0:49-0:52): `Int`
- (0:27-0:30) Test failed because `Int` is not a `Bool`.
  - (0:15-0:19): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !>, {a: Int | {b: Int}}, X)
```

### Output
```
<X = {a: Int, b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: !>, X, {a: Int | {b: Int}})
```

### Output
```
<X = {a: Int, b: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int | {b: Int}}, Int)
```

### Output
```
<>
```

### Errors
- (0:10-0:29) Test failed because an object is not an `Int`.
  - (0:31-0:34): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, Int, {a: Int | {b: Int}})
```

### Output
```
<>
```

### Errors
- (0:10-0:13) Test failed because `Int` is not an object.
  - (0:15-0:34): object

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S = R>, {p: Int | R}, {q: Bool | S})
```

### Output
```
<Type1: !, R = {q: Bool | Type1}, S = R>
```

### Errors
- (0:21-0:33) Test failed because `q:` is missing.
  - (0:36-0:37): `q:`
- (0:22-0:23) Test failed because `p:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {q: Bool | R})
```

### Output
```
<Type1: !, R = {p: Int | Type1}>
```

### Errors
- (0:14-0:22) Test failed because `q:` is missing.
  - (0:25-0:26): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | {}}, {q: Bool | R})
```

### Output
```
<Type1: !, R = {p: Int | Type1}>
```

### Errors
- (0:24-0:26) Test failed because `q:` is missing.
  - (0:30-0:31): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {p: Int | R})
```

### Output
```
<R = {}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | {}}, {p: Int | R})
```

### Output
```
<R = {}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Int | R})
```

### Output
```
<R: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {}, {})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Int, q: Bool})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Bool, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:33-0:37): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Int, q: Int})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:41-0:44): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Bool, q: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:33-0:37): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:42-0:45): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Int, q: Bool})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Bool, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Int, q: Int})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:46-0:49): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Bool, q: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:47-0:50): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Int, q: Bool | {}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Bool, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:33-0:37): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Int, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:41-0:44): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool}, {p: Bool, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:33-0:37): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:42-0:45): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Int, q: Bool | {}})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Bool, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Int, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:46-0:49): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int, q: Bool | {}}, {p: Bool, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`
- (0:22-0:26) Test failed because `Bool` is not an `Int`.
  - (0:47-0:50): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Int, q: Bool})
```

### Output
```
<R = {}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Bool, q: Bool})
```

### Output
```
<R = {}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Int, q: Int})
```

### Output
```
<R = {}>
```

### Errors
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:49-0:52): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Bool, q: Int})
```

### Output
```
<R = {}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:50-0:53): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool}, {p: Int, q: Bool | R})
```

### Output
```
<R = {}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool}, {p: Bool, q: Bool | R})
```

### Output
```
<R = {}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:37-0:41): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool}, {p: Int, q: Int | R})
```

### Output
```
<R = {}>
```

### Errors
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:45-0:48): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool}, {p: Bool, q: Int | R})
```

### Output
```
<R = {}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:37-0:41): `Bool`
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:46-0:49): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Int, q: Bool | R})
```

### Output
```
<R: !>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Bool, q: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Int, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:49-0:52): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int, q: Bool | R}, {p: Bool, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:41-0:45): `Bool`
- (0:26-0:30) Test failed because `Bool` is not an `Int`.
  - (0:50-0:53): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int, q: Bool | R}, {p: Int, q: Bool | S})
```

### Output
```
<R: !, S = R>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int, q: Bool | R}, {p: Bool, q: Bool | S})
```

### Output
```
<R: !, S = R>
```

### Errors
- (0:24-0:27) Test failed because `Int` is not a `Bool`.
  - (0:47-0:51): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int, q: Bool | R}, {p: Int, q: Int | S})
```

### Output
```
<R: !, S = R>
```

### Errors
- (0:32-0:36) Test failed because `Bool` is not an `Int`.
  - (0:55-0:58): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int, q: Bool | R}, {p: Bool, q: Int | S})
```

### Output
```
<R: !, S = R>
```

### Errors
- (0:24-0:27) Test failed because `Int` is not a `Bool`.
  - (0:47-0:51): `Bool`
- (0:32-0:36) Test failed because `Bool` is not an `Int`.
  - (0:56-0:59): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Int, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:29-0:30): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Bool, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:24-0:28): `Bool`
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:30-0:31): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Int, q: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:29-0:30): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Bool, q: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:24-0:28): `Bool`
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:30-0:31): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Int, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Bool, q: Bool})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:29-0:33): `Bool`
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:35-0:36): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Int, q: Int})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Bool, q: Int})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:29-0:33): `Bool`
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:35-0:36): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Int, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:29-0:30): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Bool, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:24-0:28): `Bool`
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:30-0:31): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Int, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:29-0:30): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int}, {p: Bool, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:24-0:28): `Bool`
- (0:10-0:18) Test failed because `q:` is missing.
  - (0:30-0:31): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Int, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Bool, q: Bool | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:29-0:33): `Bool`
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:35-0:36): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Int, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {p: Int | {}}, {p: Bool, q: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:14-0:17) Test failed because `Int` is not a `Bool`.
  - (0:29-0:33): `Bool`
- (0:20-0:22) Test failed because `q:` is missing.
  - (0:35-0:36): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Int, q: Bool})
```

### Output
```
<R = {q: Bool}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Bool, q: Bool})
```

### Output
```
<R = {q: Bool}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Int, q: Int})
```

### Output
```
<R = {q: Int}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Bool, q: Int})
```

### Output
```
<R = {q: Int}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {p: Int, q: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:14-0:22) Test failed because `q:` is missing.
  - (0:33-0:34): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {p: Bool, q: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:28-0:32): `Bool`
- (0:14-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {p: Int, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:14-0:22) Test failed because `q:` is missing.
  - (0:33-0:34): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int}, {p: Bool, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:28-0:32): `Bool`
- (0:14-0:22) Test failed because `q:` is missing.
  - (0:34-0:35): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Int, q: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:14-0:26) Test failed because `q:` is missing.
  - (0:37-0:38): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Bool, q: Bool | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`
- (0:14-0:26) Test failed because `q:` is missing.
  - (0:38-0:39): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Int, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:14-0:26) Test failed because `q:` is missing.
  - (0:37-0:38): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !>, {p: Int | R}, {p: Bool, q: Int | R})
```

### Output
```
<R: !>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`
- (0:14-0:26) Test failed because `q:` is missing.
  - (0:38-0:39): `q:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int | R}, {p: Int, q: Bool | S})
```

### Output
```
<Type1: !, R = {q: Bool | Type1}, S = Type1>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int | R}, {p: Bool, q: Bool | S})
```

### Output
```
<Type1: !, R = {q: Bool | Type1}, S = Type1>
```

### Errors
- (0:24-0:27) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int | R}, {p: Int, q: Int | S})
```

### Output
```
<Type1: !, R = {q: Int | Type1}, S = Type1>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R: !, S: !>, {p: Int | R}, {p: Bool, q: Int | S})
```

### Output
```
<Type1: !, R = {q: Int | Type1}, S = Type1>
```

### Errors
- (0:24-0:27) Test failed because `Int` is not a `Bool`.
  - (0:38-0:42): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int | {}})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int | Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:10-0:18) Test failed because an object is not an `Int`.
  - (0:30-0:33): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Int})
```

### Output
```
<>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Int, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `a:` is missing.
  - (0:29-0:30): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:19-0:20) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Int, b: void})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:29-0:30): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {a: Int, a: Bool, b: void})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `a:` is missing.
  - (0:29-0:30): `a:`
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:38-0:39): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool}, {a: Int, b: void})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `b:` is missing.
  - (0:38-0:39): `b:`
- (0:19-0:20) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: void}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:19-0:20) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, b: void}, {a: Int, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `a:` is missing.
  - (0:38-0:39): `a:`
- (0:19-0:20) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool, b: void}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:19-0:20) Test failed because `a:` is not needed.
- (0:28-0:29) Test failed because `b:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, b: void})
```

### Output
```
<T = {b: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Int, a: Bool, b: void})
```

### Output
```
<T = {a: Bool, b: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, b: void}, {a: Int | T})
```

### Output
```
<T = {b: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int, a: Bool, b: void}, {a: Int | T})
```

### Output
```
<T = {a: Bool, b: void}>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {b: void})
```

### Output
```
<Type1: !, T = {b: void | Type1}>
```

### Errors
- (0:15-0:16) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {b: void, c: Bool})
```

### Output
```
<Type1: !, T = {b: void, c: Bool | Type1}>
```

### Errors
- (0:15-0:16) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Int | T}, {a: Bool, b: void})
```

### Output
```
<T = {b: void}>
```

### Errors
- (0:18-0:21) Test failed because `Int` is not a `Bool`.
  - (0:32-0:36): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {b: void}, {a: Int | T})
```

### Output
```
<Type1: !, T = {b: void | Type1}>
```

### Errors
- (0:14-0:23) Test failed because `a:` is missing.
  - (0:26-0:27): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {b: void, c: Bool}, {a: Int | T})
```

### Output
```
<Type1: !, T = {b: void, c: Bool | Type1}>
```

### Errors
- (0:14-0:32) Test failed because `a:` is missing.
  - (0:35-0:36): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !>, {a: Bool, b: void}, {a: Int | T})
```

### Output
```
<T = {b: void}>
```

### Errors
- (0:18-0:22) Test failed because `Bool` is not an `Int`.
  - (0:38-0:41): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int}, {b: Int, c: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:18) Test failed because `b:` is missing.
  - (0:21-0:22): `b:`
- (0:10-0:18) Test failed because `c:` is missing.
  - (0:29-0:30): `c:`
- (0:11-0:12) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {a: Int, a: Bool}, {b: Int, c: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `b:` is missing.
  - (0:30-0:31): `b:`
- (0:10-0:27) Test failed because `c:` is missing.
  - (0:38-0:39): `c:`
- (0:11-0:12) Test failed because `a:` is not needed.
- (0:19-0:20) Test failed because `a:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Int, c: Bool}, {a: Int})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `a:` is missing.
  - (0:30-0:31): `a:`
- (0:11-0:12) Test failed because `b:` is not needed.
- (0:19-0:20) Test failed because `c:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<>, {b: Int, c: Bool}, {a: Int, a: Bool})
```

### Output
```
<>
```

### Errors
- (0:10-0:27) Test failed because `a:` is missing.
  - (0:30-0:31): `a:`
- (0:10-0:27) Test failed because `a:` is missing.
  - (0:38-0:39): `a:`
- (0:11-0:12) Test failed because `b:` is not needed.
- (0:19-0:20) Test failed because `c:` is not needed.

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <Y: !> {a: Y}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: X}, U = <X: !> {a: X, b: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: X}, U = <X: !, Y: !> {a: X, b: Y}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: X}, U = <X: !, Y: !> {a: X, b: Y}>
```

### Errors
- (0:64-0:65) Test failed because `{a: !, b: !}` is more general than `<X: !> {a: X, b: X}`.
  - (0:49-0:61): `{a: !, b: !}`
  - (0:18-0:30): `<X: !> {a: X, b: X}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !, Y: !> {a: X, b: Y}, U = <X: !> {a: X, b: X}>, T, U)
```

### Output
```
<T = <X: !, Y: !> {a: X, b: Y}, U = <X: !> {a: X, b: X}>
```

### Errors
- (0:64-0:65) Test failed because `{a: !, b: !}` is more general than `<Y: !> {a: Y, b: Y}`.
  - (0:24-0:36): `{a: !, b: !}`
  - (0:24-0:36): `<Y: !> {a: Y, b: Y}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !, Y: !> {a: X, b: Y}, U = <X: !, Y: !> {a: X, b: Y}>, T, U)
```

### Output
```
<T = <X: !, Y: !> {a: X, b: Y}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X}>
```

### Errors
- (0:54-0:55) Test failed because `b:` is not needed.
  - (0:25-0:26): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X, b: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = <X: !> {a: X, b: Int}>
```

### Errors
- (0:54-0:55) Test failed because `b:` is missing.
  - (0:44-0:45): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, a: Int}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X, a: Int}, U = <X: !> {a: X}>
```

### Errors
- (0:54-0:55) Test failed because `a:` is not needed.
  - (0:25-0:26): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X, a: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = <X: !> {a: X, a: Int}>
```

### Errors
- (0:54-0:55) Test failed because `a:` is missing.
  - (0:44-0:45): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, c: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, c: Int}>
```

### Errors
- (0:62-0:63) Test failed because `c:` is missing.
  - (0:52-0:53): `c:`
- (0:62-0:63) Test failed because `b:` is not needed.
  - (0:25-0:26): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, a: Int}, U = <X: !> {a: X, c: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X, a: Int}, U = <X: !> {a: X, c: Int}>
```

### Errors
- (0:62-0:63) Test failed because `c:` is missing.
  - (0:52-0:53): `c:`
- (0:62-0:63) Test failed because `a:` is not needed.
  - (0:25-0:26): `a:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, a: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, a: Int}>
```

### Errors
- (0:62-0:63) Test failed because `a:` is missing.
  - (0:52-0:53): `a:`
- (0:62-0:63) Test failed because `b:` is not needed.
  - (0:25-0:26): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, b: Bool}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, b: Bool}>
```

### Errors
- (0:63-0:64) Test failed because `Int` is not a `Bool`.
  - (0:28-0:31): `Int`
  - (0:55-0:59): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X, b: !}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = <X: !, Type1: !> {a: X, b: Type1}>
```

### Errors
- (0:60-0:61) Test failed because `{a: !, b: !}` is more general than `{a: !, b: Int}`.
  - (0:45-0:57): `{a: !, b: !}`
  - (0:18-0:32): `{a: !, b: Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: !}, U = <X: !> {a: X, b: Int}>, T, U)
```

### Output
```
<T = <X: !, Type1: !> {a: X, b: Type1}, U = <X: !> {a: X, b: Int}>
```

### Errors
- (0:60-0:61) Test failed because `{a: !, b: !}` is more general than `{a: !, b: Int}`.
  - (0:18-0:30): `{a: !, b: !}`
  - (0:18-0:30): `{a: !, b: Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | {}}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X | {}}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | {}}, U = <X: !> {a: X | {}}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int}, U = <X: !> {a: X | {b: Int}}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | {b: Int}}, U = <X: !> {a: X, b: Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X, b: Int | {}}, U = <X: !> {a: X | {b: Int}}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | {b: Int}}, U = <X: !> {a: X, b: Int | {}}>, T, U)
```

### Output
```
<T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | <Y: !> {b: Y}}, U = <X: !> {a: X | <Y: !> {b: Y}}>, T, U)
```

### Output
```
<T = <Type1: <Y: !> {b: Y}, X: !> {a: X | Type1}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !, R = <Y: !> {b: Y}> {a: X | R}, U = <X: !, R = <Y: !> {b: Y}> {a: X | R}>, T, U)
```

### Output
```
<T = <R = <Y: !> {b: Y}, X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | !}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !, Type1: !> {a: X | Type1}, U = <X: !> {a: X}>
```

### Errors
- (0:50-0:51) Test failed because `{a: ! | !}` is more general than `{a: !}`.
  - (0:18-0:28): `{a: ! | !}`
  - (0:18-0:28): `{a: !}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X | !}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = <X: !, Type1: !> {a: X | Type1}>
```

### Errors
- (0:50-0:51) Test failed because `{a: ! | !}` is more general than `{a: !}`.
  - (0:37-0:47): `{a: ! | !}`
  - (0:18-0:24): `{a: !}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | !}, U = <X: !> {a: X | !}>, T, U)
```

### Output
```
<T = <Type1: !, X: !> {a: X | Type1}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | Int}, U = <X: !> {a: X}>, T, U)
```

### Output
```
<T = <X: !> {a: X | Int}, U = <X: !> {a: X}>
```

### Errors
- (0:52-0:53) Test failed because `Int` is not an object.
  - (0:26-0:29): `Int`
  - (0:43-0:49): object

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X}, U = <X: !> {a: X | Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X}, U = <X: !> {a: X | Int}>
```

### Errors
- (0:52-0:53) Test failed because an object is not an `Int`.
  - (0:18-0:24): object
  - (0:45-0:48): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | Int}, U = <X: !> {a: X | Int}>, T, U)
```

### Output
```
<T = <X: !> {a: X | Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | Int}, U = <X: !> {a: X | Bool}>, T, U)
```

### Output
```
<T = <X: !> {a: X | Int}, U = <X: !> {a: X | Bool}>
```

### Errors
- (0:59-0:60) Test failed because `Int` is not a `Bool`.
  - (0:26-0:29): `Int`
  - (0:51-0:55): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | Int}, U = <X: !> {a: X | !}>, T, U)
```

### Output
```
<T = <X: !> {a: X | Int}, U = <X: !, Type1: !> {a: X | Type1}>
```

### Errors
- (0:56-0:57) Test failed because `{a: ! | !}` is more general than `{a: ! | Int}`.
  - (0:43-0:53): `{a: ! | !}`
  - (0:18-0:30): `{a: ! | Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> {a: X | !}, U = <X: !> {a: X | Int}>, T, U)
```

### Output
```
<T = <X: !, Type1: !> {a: X | Type1}, U = <X: !> {a: X | Int}>
```

### Errors
- (0:56-0:57) Test failed because `{a: ! | !}` is more general than `{a: ! | Int}`.
  - (0:18-0:28): `{a: ! | !}`
  - (0:18-0:28): `{a: ! | Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = <Y: !> {b: Y}, T = <X: !> {a: X | R}, U = <X: !> {a: X | R}>, T, U)
```

### Output
```
<R = <Y: !> {b: Y}, T = <X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = <Y: !> {b: Y}, S = <Y: !> {b: Y}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>, T, U)
```

### Output
```
<R = <Y: !> {b: Y}, S = R, T = <X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = <Y: !> {b: Y}, S = <Y: !> {c: Y}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>, T, U)
```

### Output
```
<R = <Y: !> {b: Y}, S = <Y: !> {c: Y}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>
```

### Errors
- (0:92-0:93) Test failed because `c:` is missing.
  - (0:38-0:39): `c:`
- (0:92-0:93) Test failed because `b:` is not needed.
  - (0:19-0:20): `b:`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = <Y: !> {b: Y}, T = <X: !> {a: X | R}, U = <X: !> {a: X | <Y: !> {b: Y}}>, T, U)
```

### Output
```
<R = <Y: !> {b: Y}, T = <X: !> {a: X | R}, U = <X: !, Type1: <Y: !> {b: Y}> {a: X | Type1}>
```

### Errors
- (0:85-0:86) Test failed because `{a: !, b: !}` is more general than `{a: ! | R}`.
  - (0:60-0:82): `{a: !, b: !}`
  - (0:37-0:47): `{a: ! | R}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = <Y: !> {b: Y}, T = <X: !> {a: X | <Y: !> {b: Y}}, U = <X: !> {a: X | R}>, T, U)
```

### Output
```
<R = <Y: !> {b: Y}, T = <X: !, Type1: <Y: !> {b: Y}> {a: X | Type1}, U = <X: !> {a: X | R}>
```

### Errors
- (0:85-0:86) Test failed because `{a: !, b: !}` is more general than `{a: ! | R}`.
  - (0:37-0:59): `{a: !, b: !}`
  - (0:37-0:59): `{a: ! | R}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, T = <X: !> {a: X | R}, U = <X: !> {a: X | R}>, T, U)
```

### Output
```
<R = {b: Int}, T = <X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, T = <X: !> {a: X | R}, U = <X: !> {a: X, b: Int}>, T, U)
```

### Output
```
<R = {b: Int}, T = <X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, T = <X: !> {a: X, b: Int}, U = <X: !> {a: X | R}>, T, U)
```

### Output
```
<R = {b: Int}, T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, T = <X: !> {a: X | R}, U = <X: !> {a: X | {b: Int}}>, T, U)
```

### Output
```
<R = {b: Int}, T = <X: !> {a: X | R}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, T = <X: !> {a: X | {b: Int}}, U = <X: !> {a: X | R}>, T, U)
```

### Output
```
<R = {b: Int}, T = <X: !> {a: X, b: Int}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, S = {b: Bool}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>, T, U)
```

### Output
```
<R = {b: Int}, S = {b: Bool}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>
```

### Errors
- (0:83-0:84) Test failed because `Int` is not a `Bool`.
  - (0:15-0:18): `Int`
  - (0:29-0:33): `Bool`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: Int}, S = {b: !}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>, T, U)
```

### Output
```
<R = {b: Int}, S = <Type1: !> {b: Type1}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>
```

### Errors
- (0:80-0:81) Test failed because `{b: !}` is more general than `{b: Int}`.
  - (0:25-0:31): `{b: !}`
  - (0:11-0:19): `{b: Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<R = {b: !}, S = {b: Int}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>, T, U)
```

### Output
```
<R = <Type1: !> {b: Type1}, S = {b: Int}, T = <X: !> {a: X | R}, U = <X: !> {a: X | S}>
```

### Errors
- (0:80-0:81) Test failed because `{b: !}` is more general than `{b: Int}`.
  - (0:11-0:17): `{b: !}`
  - (0:23-0:31): `{b: Int}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<T>(Int) -> fun(T) -> T, B: fun<T, U>(U) -> fun(T) -> T>, A, B)
```

### Output
```
<A = fun<T>(Int) -> fun(T) -> T, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = fun<T>(Int) -> fun(T) -> T, B: fun<T, U>(U) -> fun(T) -> T>, B, A)
```

### Output
```
<B = fun<T>(Int) -> fun(T) -> T, A = B>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<T>(Int) -> fun(T) -> T, B = fun<T, U>(U) -> fun(T) -> T>, A, B)
```

### Output
```
<A: fun<T>(Int) -> fun(T) -> T, B = fun<T, U>(U) -> fun(T) -> T>
```

### Errors
- (0:72-0:73) Test failed because `fun<T, U>(U) -> fun(T) -> T` is more general than `fun<T>(Int) -> fun(T) -> T`.
  - (0:42-0:69): `fun<T, U>(U) -> fun(T) -> T`
  - (0:10-0:36): `fun<T>(Int) -> fun(T) -> T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: fun<T>(Int) -> fun(T) -> T, B = fun<T, U>(U) -> fun(T) -> T>, B, A)
```

### Output
```
<A: fun<T>(Int) -> fun(T) -> T, B = fun<T, U>(U) -> fun(T) -> T>
```

### Errors
- (0:72-0:73) Test failed because `fun<T, U>(U) -> fun(T) -> T` is more general than `fun<T>(Int) -> fun(T) -> T`.
  - (0:42-0:69): `fun<T, U>(U) -> fun(T) -> T`
  - (0:10-0:36): `fun<T>(Int) -> fun(T) -> T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<F: fun<T>(T) -> T, G: fun(Int) -> Int, H = fun<T>(T) -> T>, fun(F) -> F, fun(G) -> H)
```

### Output
```
<F: fun(Int) -> Int, G = F, H = fun<T>(T) -> T>
```

### Errors
- (0:77-0:78) Test failed because `fun<T>(T) -> T` is more general than `fun(Int) -> Int`.
  - (0:50-0:64): `fun<T>(T) -> T`
  - (0:29-0:44): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<F: fun<T>(T) -> T, G: fun(Int) -> Int, H = fun<T>(T) -> T>, fun(G) -> H, fun(F) -> F)
```

### Output
```
<G: fun(Int) -> Int, F = G, H = fun<T>(T) -> T>
```

### Errors
- (0:77-0:78) Test failed because `fun<T>(T) -> T` is more general than `fun(Int) -> Int`.
  - (0:50-0:64): `fun<T>(T) -> T`
  - (0:29-0:44): `fun(Int) -> Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T>, T, T)
```

### Output
```
<T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T>, fun(T) -> T, fun(T) -> T)
```

### Output
```
<T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: !>, T, U)
```

### Output
```
<T, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: !>, U, T)
```

### Output
```
<T, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U = !>, T, U)
```

### Output
```
<T, U = !>
```

### Errors
- (0:18-0:19) Test failed because `!` is more general than `T`.
  - (0:14-0:15): `!`
  - (0:7-0:8): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U = !>, U, T)
```

### Output
```
<T, U = !>
```

### Errors
- (0:18-0:19) Test failed because `!` is more general than `T`.
  - (0:14-0:15): `!`
  - (0:7-0:8): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U>, T, U)
```

### Output
```
<T = !, U>
```

### Errors
- (0:18-0:19) Test failed because `!` is more general than `U`.
  - (0:11-0:12): `!`
  - (0:14-0:15): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U>, U, T)
```

### Output
```
<T = !, U>
```

### Errors
- (0:18-0:19) Test failed because `!` is more general than `U`.
  - (0:11-0:12): `!`
  - (0:14-0:15): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: <Z> !>, T, U)
```

### Output
```
<T, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: <Z> !>, U, T)
```

### Output
```
<T, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U = <Z> !>, T, U)
```

### Output
```
<T, U = !>
```

### Errors
- (0:22-0:23) Test failed because `!` is more general than `T`.
  - (0:18-0:19): `!`
  - (0:7-0:8): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U = <Z> !>, U, T)
```

### Output
```
<T, U = !>
```

### Errors
- (0:22-0:23) Test failed because `!` is more general than `T`.
  - (0:18-0:19): `!`
  - (0:7-0:8): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: _>, T, U)
```

### Output
```
<T: <Type1> Type1, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: _>, U, T)
```

### Output
```
<U: <Type1> Type1, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: <V> V>, T, U)
```

### Output
```
<T: <V> V, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: <V> V>, U, T)
```

### Output
```
<U: <V> V, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B: <Y> Y>, A, B)
```

### Output
```
<A = !, B: <Y> Y>
```

### Errors
- (0:25-0:26) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:21-0:22): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = !, B: <Y> Y>, B, A)
```

### Output
```
<A = !, B: <Y> Y>
```

### Errors
- (0:25-0:26) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:21-0:22): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> X, U: <Y> Y>, T, U)
```

### Output
```
<T = !, U: <Y> Y>
```

### Errors
- (0:32-0:33) Test failed because `!` is more general than `_`.
  - (0:15-0:16): `!`
  - (0:28-0:29): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> X, U: <Y> Y>, U, T)
```

### Output
```
<T = !, U: <Y> Y>
```

### Errors
- (0:32-0:33) Test failed because `!` is more general than `_`.
  - (0:15-0:16): `!`
  - (0:28-0:29): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X = !> X, U: <Y> Y>, T, U)
```

### Output
```
<T = !, U: <Y> Y>
```

### Errors
- (0:33-0:34) Test failed because `!` is more general than `_`.
  - (0:16-0:17): `!`
  - (0:29-0:30): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X = !> X, U: <Y> Y>, U, T)
```

### Output
```
<T = !, U: <Y> Y>
```

### Errors
- (0:33-0:34) Test failed because `!` is more general than `_`.
  - (0:16-0:17): `!`
  - (0:29-0:30): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> fun(X) -> void, U: <Y> fun(Y) -> void>, T, U)
```

### Output
```
<T = fun<X>(X) -> void, U: <Y> fun(Y) -> void>
```

### Errors
- (0:58-0:59) Test failed because `fun<X>(X) -> void` is more general than `fun(_) -> void`.
  - (0:18-0:32): `fun<X>(X) -> void`
  - (0:41-0:55): `fun(_) -> void`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X: !> fun(X) -> void, U: <Y> fun(Y) -> void>, U, T)
```

### Output
```
<T = fun<X>(X) -> void, U: <Y> fun(Y) -> void>
```

### Errors
- (0:58-0:59) Test failed because `fun<X>(X) -> void` is more general than `fun(_) -> void`.
  - (0:18-0:32): `fun<X>(X) -> void`
  - (0:41-0:55): `fun(_) -> void`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X = !> fun(X) -> void, U: <Y> fun(Y) -> void>, T, U)
```

### Output
```
<T = fun<X = !>(X) -> void, U: <Y> fun(Y) -> void>
```

### Errors
- (0:59-0:60) Test failed because `!` is more general than `Y`.
  - (0:16-0:17): `!`
  - (0:39-0:40): `Y`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <X = !> fun(X) -> void, U: <Y> fun(Y) -> void>, U, T)
```

### Output
```
<T = fun<X = !>(X) -> void, U: <Y> fun(Y) -> void>
```

### Errors
- (0:59-0:60) Test failed because `!` is more general than `Y`.
  - (0:16-0:17): `!`
  - (0:39-0:40): `Y`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U: _>, T, U)
```

### Output
```
<T = !, U: <Type1> Type1>
```

### Errors
- (0:21-0:22) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:17-0:18): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U: _>, U, T)
```

### Output
```
<T = !, U: <Type1> Type1>
```

### Errors
- (0:21-0:22) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:17-0:18): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U: <V> V>, T, U)
```

### Output
```
<T = !, U: <V> V>
```

### Errors
- (0:25-0:26) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:21-0:22): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U: <V> V>, U, T)
```

### Output
```
<T = !, U: <V> V>
```

### Errors
- (0:25-0:26) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:21-0:22): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: _>, T, U)
```

### Output
```
<T: <Type1> Type1, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U: _>, U, T)
```

### Output
```
<U: <Type1> Type1, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U = _>, T, U)
```

### Output
```
<T = <Type1> Type1, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, U = _>, U, T)
```

### Output
```
<U = <Type1> Type1, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U = _>, T, U)
```

### Output
```
<T = !, U = <Type1> Type1>
```

### Errors
- (0:22-0:23) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:18-0:19): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = !, U = _>, U, T)
```

### Output
```
<T = !, U = <Type1> Type1>
```

### Errors
- (0:22-0:23) Test failed because `!` is more general than `_`.
  - (0:11-0:12): `!`
  - (0:18-0:19): `_`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A: !> {x: A}, U: <B> {x: B}>, T, U)
```

### Output
```
<T: <B> {x: B}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A: !> {x: A}, U: <B> {x: B}>, U, T)
```

### Output
```
<U: <B> {x: B}, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <A: !> {x: A}, U: <B> {x: B}>, T, U)
```

### Output
```
<T = <A: !> {x: A}, U: <B> {x: B}>
```

### Errors
- (0:42-0:43) Test failed because `{x: !}` is more general than `{x: _}`.
  - (0:18-0:24): `{x: !}`
  - (0:33-0:39): `{x: _}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <A: !> {x: A}, U: <B> {x: B}>, U, T)
```

### Output
```
<T = <A: !> {x: A}, U: <B> {x: B}>
```

### Errors
- (0:42-0:43) Test failed because `{x: !}` is more general than `{x: _}`.
  - (0:18-0:24): `{x: !}`
  - (0:33-0:39): `{x: _}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A: !> {x: A}, U = <B> {x: B}>, T, U)
```

### Output
```
<T = <B> {x: B}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A: !> {x: A}, U = <B> {x: B}>, U, T)
```

### Output
```
<U = <B> {x: B}, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <A: !> {x: A}, U = <B> {x: B}>, T, U)
```

### Output
```
<T = <A: !> {x: A}, U = <B> {x: B}>
```

### Errors
- (0:43-0:44) Test failed because `{x: !}` is more general than `{x: _}`.
  - (0:18-0:24): `{x: !}`
  - (0:18-0:24): `{x: _}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T = <A: !> {x: A}, U = <B> {x: B}>, U, T)
```

### Output
```
<T = <A: !> {x: A}, U = <B> {x: B}>
```

### Errors
- (0:43-0:44) Test failed because `{x: !}` is more general than `{x: _}`.
  - (0:18-0:24): `{x: !}`
  - (0:34-0:40): `{x: _}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {outer: {inner: T}}, B: {outer: <T> {inner: T}}>, A, B)
```

### Output
```
<A: <T> {outer: {inner: T}}, B: <Type1: <T> {inner: T}> {outer: Type1}>
```

### Errors
- (0:64-0:65) Test failed because `T` is not `T2`.
  - (0:11-0:12): `T`
  - (0:47-0:48): `T2`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {outer: {inner: T}}, B: {outer: <T> {inner: T}}>, B, A)
```

### Output
```
<A: <T> {outer: {inner: T}}, B: <Type1: <T> {inner: T}> {outer: Type1}>
```

### Errors
- (0:64-0:65) Test failed because `T2` is not `T`.
  - (0:47-0:48): `T2`
  - (0:11-0:12): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {outer: {inner: T}}, B: <T> {outer: {inner: T}}>, A, B)
```

### Output
```
<A: <T> {outer: {inner: T}}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: {outer: <T> {inner: T}}, B: {outer: <T> {inner: T}}>, A, B)
```

### Output
```
<A: <Type1: <T> {inner: T}> {outer: Type1}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {foo: T}, B: <T> {foo: T}>, A, B)
```

### Output
```
<A: <T> {foo: T}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {foo: T, bar: T}, B: <T, U> {foo: T, bar: U}>, A, B)
```

### Output
```
<A: <T> {foo: T, bar: T}, B: <T, U> {foo: T, bar: U}>
```

### Errors
- (0:61-0:62) Test failed because `T` is not `T2`.
  - (0:11-0:12): `T`
  - (0:36-0:37): `T2`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <T> {foo: T, bar: T}, B: <T, U> {foo: T, bar: U}>, B, A)
```

### Output
```
<A: <T> {foo: T, bar: T}, B: <T, U> {foo: T, bar: U}>
```

### Errors
- (0:61-0:62) Test failed because `T` is not `U`.
  - (0:36-0:37): `T`
  - (0:39-0:40): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int, B = A, C>, B, C)
```

### Output
```
<A = Int, B = A, C>
```

### Errors
- (0:27-0:28) Test failed because `Int` is not `C`.
  - (0:11-0:14): `Int`
  - (0:23-0:24): `C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int, B = A, C>, C, B)
```

### Output
```
<A = Int, B = A, C>
```

### Errors
- (0:27-0:28) Test failed because `C` is not an `Int`.
  - (0:23-0:24): `C`
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int, B = <Z> A, C>, B, C)
```

### Output
```
<A = Int, B = A, C>
```

### Errors
- (0:31-0:32) Test failed because `Int` is not `C`.
  - (0:11-0:14): `Int`
  - (0:27-0:28): `C`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = Int, B = <Z> A, C>, C, B)
```

### Output
```
<A = Int, B = A, C>
```

### Errors
- (0:31-0:32) Test failed because `C` is not an `Int`.
  - (0:27-0:28): `C`
  - (0:11-0:14): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <T> T, B>, A, B)
```

### Output
```
<A = <T> T, B>
```

### Errors
- (0:22-0:23) Test failed because `T` is not `B`.
  - (0:12-0:13): `T`
  - (0:18-0:19): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <T> T, B>, B, A)
```

### Output
```
<A = <T> T, B>
```

### Errors
- (0:22-0:23) Test failed because `B` is not `T`.
  - (0:18-0:19): `B`
  - (0:12-0:13): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int, B>, A, B)
```

### Output
```
<A: Int, B>
```

### Errors
- (0:19-0:20) Test failed because `Int` is not `B`.
  - (0:10-0:13): `Int`
  - (0:15-0:16): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: Int, B>, B, A)
```

### Output
```
<A: Int, B>
```

### Errors
- (0:19-0:20) Test failed because `B` is not an `Int`.
  - (0:15-0:16): `B`
  - (0:10-0:13): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, A = <T> T, B>, A, B)
```

### Output
```
<A = <T> T, B, T>
```

### Errors
- (0:25-0:26) Test failed because `T2` is not `B`.
  - (0:15-0:16): `T2`
  - (0:21-0:22): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, A = <T> T, B>, B, A)
```

### Output
```
<A = <T> T, B, T>
```

### Errors
- (0:25-0:26) Test failed because `B` is not `T2`.
  - (0:21-0:22): `B`
  - (0:15-0:16): `T2`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, A = <T> T, B>, A, B)
```

### Output
```
<A = <T> T, B, T: !>
```

### Errors
- (0:28-0:29) Test failed because `T2` is not `B`.
  - (0:18-0:19): `T2`
  - (0:24-0:25): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, A = <T> T, B>, B, A)
```

### Output
```
<A = <T> T, B, T: !>
```

### Errors
- (0:28-0:29) Test failed because `B` is not `T2`.
  - (0:24-0:25): `B`
  - (0:18-0:19): `T2`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A, B>, A, B)
```

### Output
```
<A, B>
```

### Errors
- (0:14-0:15) Test failed because `A` is not `B`.
  - (0:7-0:8): `A`
  - (0:10-0:11): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A, B>, B, A)
```

### Output
```
<A, B>
```

### Errors
- (0:14-0:15) Test failed because `B` is not `A`.
  - (0:10-0:11): `B`
  - (0:7-0:8): `A`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> X, B: <Y> Y>, A, B)
```

### Output
```
<A: <X> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <X> X, B: <Y> Y>, A, B)
```

### Output
```
<A = <X> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> X, B = <Y> Y>, A, B)
```

### Output
```
<A = <X> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A = <X> X, B = <Y> Y>, A, B)
```

### Output
```
<A = <X> X, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y}, B: <X, Y> {x: X, y: Y}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y}, B: <X, Y> {x: Y, y: X}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y}, B: <T, U> {x: T, y: U}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y}, B: <U, T> {x: T, y: U}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y, z: Int}, B: <T, U, Z: !> {x: T, y: U, z: Z}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y, z: Int}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y, z: Int}, B: <Z: !, T, U> {x: T, y: U, z: Z}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y, z: Int}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X, Y> {x: X, y: Y, z: Int}, B: <T, Z: !, U> {x: T, y: U, z: Z}>, A, B)
```

### Output
```
<A: <X, Y> {x: X, y: Y, z: Int}, B = A>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X}, B: <T, U> {x: T, y: U}>, A, B)
```

### Output
```
<A: <X> {x: X, y: X}, B: <T, U> {x: T, y: U}>
```

### Errors
- (0:53-0:54) Test failed because `X` is not `U`.
  - (0:11-0:12): `X`
  - (0:35-0:36): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X}, B: <T, U> {x: T, y: U}>, B, A)
```

### Output
```
<A: <X> {x: X, y: X}, B: <T, U> {x: T, y: U}>
```

### Errors
- (0:53-0:54) Test failed because `U` is not `T`.
  - (0:35-0:36): `U`
  - (0:32-0:33): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U, V> {x: T, y: U, z: V}>, A, B)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U, V> {x: T, y: U, z: V}>
```

### Errors
- (0:68-0:69) Test failed because `X` is not `U`.
  - (0:11-0:12): `X`
  - (0:41-0:42): `U`
- (0:68-0:69) Test failed because `X` is not `V`.
  - (0:11-0:12): `X`
  - (0:44-0:45): `V`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U, V> {x: T, y: U, z: V}>, B, A)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U, V> {x: T, y: U, z: V}>
```

### Errors
- (0:68-0:69) Test failed because `U` is not `T`.
  - (0:41-0:42): `U`
  - (0:38-0:39): `T`
- (0:68-0:69) Test failed because `V` is not `T`.
  - (0:44-0:45): `V`
  - (0:38-0:39): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: T, z: U}>, A, B)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: T, z: U}>
```

### Errors
- (0:65-0:66) Test failed because `X` is not `U`.
  - (0:11-0:12): `X`
  - (0:41-0:42): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: T, z: U}>, B, A)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: T, z: U}>
```

### Errors
- (0:65-0:66) Test failed because `U` is not `T`.
  - (0:41-0:42): `U`
  - (0:38-0:39): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: U, z: T}>, A, B)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: U, z: T}>
```

### Errors
- (0:65-0:66) Test failed because `X` is not `U`.
  - (0:11-0:12): `X`
  - (0:41-0:42): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: U, z: T}>, B, A)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: T, y: U, z: T}>
```

### Errors
- (0:65-0:66) Test failed because `U` is not `T`.
  - (0:41-0:42): `U`
  - (0:38-0:39): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: U, y: T, z: U}>, A, B)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: U, y: T, z: U}>
```

### Errors
- (0:65-0:66) Test failed because `X` is not `T`.
  - (0:11-0:12): `X`
  - (0:38-0:39): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: U, y: T, z: U}>, B, A)
```

### Output
```
<A: <X> {x: X, y: X, z: X}, B: <T, U> {x: U, y: T, z: U}>
```

### Errors
- (0:65-0:66) Test failed because `T` is not `U`.
  - (0:38-0:39): `T`
  - (0:41-0:42): `U`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X>, X, Int)
```

### Output
```
<X>
```

### Errors
- (0:11-0:12) Test failed because `X` is not an `Int`.
  - (0:7-0:8): `X`
  - (0:14-0:17): `Int`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X>, Int, X)
```

### Output
```
<X>
```

### Errors
- (0:11-0:14) Test failed because `Int` is not `X`.
  - (0:7-0:8): `X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X>, X, fun(Int) -> Int)
```

### Output
```
<X>
```

### Errors
- (0:11-0:12) Test failed because `X` is not a function.
  - (0:7-0:8): `X`
  - (0:14-0:29): function

--------------------------------------------------------------------------------

### Input
```ite
unify(<X>, fun(Int) -> Int, X)
```

### Output
```
<X>
```

### Errors
- (0:11-0:26) Test failed because a function is not `X`.
  - (0:7-0:8): `X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<U: <T: <X> X> T, V: <Y> Y>, U, V)
```

### Output
```
<U: <X> X, V = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<U: <T: <X> X> T, V: <Y> Y>, V, U)
```

### Output
```
<V: <Y> Y, U = V>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: <X> X>, T, U)
```

### Output
```
<T, U: <X> X>
```

### Errors
- (0:21-0:22) Test failed because `T` is not `X`.
  - (0:7-0:8): `T`
  - (0:14-0:15): `X`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T, U: <X> X>, U, T)
```

### Output
```
<T, U: <X> X>
```

### Errors
- (0:21-0:22) Test failed because `X` is not `T`.
  - (0:14-0:15): `X`
  - (0:7-0:8): `T`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: D, b: E, c: F}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type3: <C> {a: A, b: B, c: C}> {inner: Type3}> {outer: Type1}, U = T>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: F, b: E, c: D}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <D, Type1: <E, Type1: <F> {a: F, b: E, c: D}> {inner: Type1}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `A` is not `F`.
  - (0:11-0:12): `A`
  - (0:88-0:89): `F`
- (0:114-0:115) Test failed because `C` is not `D`.
  - (0:35-0:36): `C`
  - (0:64-0:65): `D`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: E, b: F, c: D}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <D, Type1: <E, Type1: <F> {a: E, b: F, c: D}> {inner: Type1}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `A` is not `E`.
  - (0:11-0:12): `A`
  - (0:76-0:77): `E`
- (0:114-0:115) Test failed because `B` is not `F`.
  - (0:23-0:24): `B`
  - (0:88-0:89): `F`
- (0:114-0:115) Test failed because `C` is not `D`.
  - (0:35-0:36): `C`
  - (0:64-0:65): `D`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: E, b: E, c: F}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <Type1: <E, Type1: <F> {a: E, b: E, c: F}> {inner: Type1}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `A` is not `E`.
  - (0:11-0:12): `A`
  - (0:76-0:77): `E`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: D, b: E, c: E}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <D, Type1: <E> {inner: {a: D, b: E, c: E}}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `C` is not `B`.
  - (0:35-0:36): `C`
  - (0:23-0:24): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: D, b: E, c: D}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <D, Type1: <E> {inner: {a: D, b: E, c: D}}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `C` is not `A`.
  - (0:35-0:36): `C`
  - (0:11-0:12): `A`

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: <A> {outer: <B> {inner: <C> {a: A, b: B, c: C}}}, U: <D> {outer: <E> {inner: <F> {a: F, b: E, c: F}}}>, T, U)
```

### Output
```
<T: <A, Type1: <B, Type1: <C> {a: A, b: B, c: C}> {inner: Type1}> {outer: Type1}, U: <Type1: <E, Type1: <F> {a: F, b: E, c: F}> {inner: Type1}> {outer: Type1}>
```

### Errors
- (0:114-0:115) Test failed because `A` is not `F`.
  - (0:11-0:12): `A`
  - (0:88-0:89): `F`

--------------------------------------------------------------------------------

### Input
```ite
unify(<V: !, T: <X> {a: X, b: X}, U: <Y> {a: Y, b: V}>, T, U)
```

### Output
```
<X, T: {a: X, b: X}, U = T, V = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<V: !, T: <X> {a: X, b: X}, U: <Y> {a: Y, b: V}>, U, T)
```

### Output
```
<Y, V = Y, U: {a: Y, b: V}, T = U>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<F: fun(_) -> !, G: fun<T>(T) -> T>, F, G)
```

### Output
```
<F: <Type1> fun(Type1) -> Type1, G = F>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<F: fun(_) -> !, G: fun<T>(T) -> T>, G, F)
```

### Output
```
<G: <Type1> fun(Type1) -> Type1, F = G>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, F: fun(_) -> T, G: fun<T>(T) -> T>, F, G)
```

### Output
```
<Type1, T = Type1, F: fun(Type1) -> T, G = F>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<T: !, F: fun(_) -> T, G: fun<T>(T) -> T>, G, F)
```

### Output
```
<Type1, G: fun(Type1) -> Type1, F = G, T = Type1>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A1> {p: A1}, Y: <A2> {p: A2}>, X, Y)
```

### Output
```
<X: <A1> {p: A1}, Y = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A1> {p: A1, q: A1}, Y: <A2, B> {p: A2, q: B}>, X, Y)
```

### Output
```
<X: <A1> {p: A1, q: A1}, Y: <A2, B> {p: A2, q: B}>
```

### Errors
- (0:58-0:59) Test failed because `A1` is not `B`.
  - (0:11-0:13): `A1`
  - (0:39-0:40): `B`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A1> {p: A1, q: A1}, Y: <A2, B> {p: A2, q: B}>, Y, X)
```

### Output
```
<X: <A1> {p: A1, q: A1}, Y: <A2, B> {p: A2, q: B}>
```

### Errors
- (0:58-0:59) Test failed because `B` is not `A2`.
  - (0:39-0:40): `B`
  - (0:35-0:37): `A2`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: !> {t: {t: A}}, Y: {t: <B> {t: B}}>, X, Y)
```

### Output
```
<X: <B> {t: {t: B}}, Y = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: !> {t: {t: A}}, Y: {t: <B> {t: B}}>, Y, X)
```

### Output
```
<Y: <B> {t: {t: B}}, X = Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: !> {t: {t: A}}, Y: {t: <A: !> {t: A}}>, X, Y)
```

### Output
```
<X = <A: !> {t: {t: A}}, Y = X>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X = <A: !> {t: {t: A}}, Y: {t: <A: !> {t: A}}>, Y, X)
```

### Output
```
<Y = <A: !> {t: {t: A}}, X = Y>
```

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: !> {t: {t: A}}, Y = {t: <A: !> {t: A}}>, X, Y)
```

### Output
```
<X: <A: !> {t: {t: A}}, Y = <Type1: <A: !> {t: A}> {t: Type1}>
```

### Errors
- (0:55-0:56) Test failed because `{t: {t: !}}` is more general than `{t: {t: !}}`.
  - (0:34-0:52): `{t: {t: !}}`
  - (0:17-0:28): `{t: {t: !}}`

--------------------------------------------------------------------------------

### Input
```ite
unify(<X: <A: !> {t: {t: A}}, Y = {t: <A: !> {t: A}}>, Y, X)
```

### Output
```
<X: <A: !> {t: {t: A}}, Y = <Type1: <A: !> {t: A}> {t: Type1}>
```

### Errors
- (0:55-0:56) Test failed because `{t: {t: !}}` is more general than `{t: {t: !}}`.
  - (0:34-0:52): `{t: {t: !}}`
  - (0:17-0:28): `{t: {t: !}}`

--------------------------------------------------------------------------------
