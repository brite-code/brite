...
testData =
  ...
  , ("infer(<>, (add1: fun(Int) -> Int), (fun(x) { let x = (x: fun<A>(A) -> A); x(x) })(add1))", "(<>, fun<A>(A) -> A)", ["(0:82-0:86) Can not call `fun(x) {}` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:57-0:71): `fun<A>(A) -> A`, (0:17-0:32): `fun(Int) -> Int`]"])
  ...
  , ("infer(<>, (), nope)", "(<>, !)", ["(0:14-0:18) Can not find `nope`."])
  , ("infer(<>, (), true)", "(<>, Bool)", [])
  ...
  , ("infer(<>, (), ((fun(x) { x }: fun(Bool) -> Bool): fun<A>(A) -> A))", "(<>, fun<A>(A) -> A)", ["(0:30-0:47) Can not change type of `fun(x) {}` because `fun<A>(A) -> A` is more general than `fun(Bool) -> Bool`. [(0:50-0:64): `fun<A>(A) -> A`]"])
  ...
  , ("infer(<>, (), fun(x) { let x = (x: fun<A>(A) -> A); x(x) })", "(<>, fun<Type1 = fun<A>(A) -> A, Type2: fun<A>(A) -> A>(Type1) -> Type2)", [])
  , ("infer(<>, (add1: fun(Int) -> Int), (fun(x) { let x = (x: fun<A>(A) -> A); x(x) })(add1))", "(<>, fun<A>(A) -> A)", ["(0:82-0:86) Can not call `fun(x) {}` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:57-0:71): `fun<A>(A) -> A`, (0:17-0:32): `fun(Int) -> Int`]"])
  , ("infer(<>, (add1: fun(Int) -> Int), (fun(x) { let x = (x: fun<A>(A) -> A); x(x) }: fun<A = fun<A>(A) -> A>(A) -> A)(add1))", "(<>, fun<A>(A) -> A)", ["(0:115-0:119) Can not call `fun(x) {}` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:90-0:104): `fun<A>(A) -> A`, (0:17-0:32): `fun(Int) -> Int`]"])
  , ("infer(<>, (auto: fun<A = fun<A>(A) -> A>(A) -> A, add1: fun(Int) -> Int), auto(add1))", "(<>, fun<A>(A) -> A)", ["(0:79-0:83) Can not call `auto` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:25-0:39): `fun<A>(A) -> A`, (0:56-0:71): `fun(Int) -> Int`]"])
  ...
  , ("infer(<>, (), do { let id = fun(x) { x }; (id: fun<X>(X) -> Int) })", "(<>, fun<X>(X) -> Int)", ["(0:43-0:45) Can not change type of `id` because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`. [(0:47-0:63): `fun<X>(X) -> Int`, (0:28-0:40): `fun(Int) -> Int`]"])
  , ("infer(<>, (), do { let id = fun(x) { x }; let id = (id: fun<X>(X) -> Int); (id: fun<X>(X) -> X) })", "(<>, fun<X>(X) -> X)", ["(0:52-0:54) Can not change type of `id` because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`. [(0:56-0:72): `fun<X>(X) -> Int`, (0:28-0:40): `fun(Int) -> Int`]", "(0:76-0:78) Can not change type of `id` because `fun<X>(X) -> X` is more general than `fun(Int) -> Int`. [(0:80-0:94): `fun<X>(X) -> X`, (0:56-0:72): `fun(Int) -> Int`]"])
  ...
  , ("infer(<>, (auto: fun<A = fun<A>(A) -> A>(A) -> A, add1: fun(Int) -> Int), auto(add1))", "(<>, fun<A>(A) -> A)", ["(0:79-0:83) Can not call `auto` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:25-0:39): `fun<A>(A) -> A`, (0:56-0:71): `fun(Int) -> Int`]"])
  , ("infer(<>, (add1: fun(Int) -> Int), do { let id = fun(x) { x }; (id: fun<X>(X) -> Int); id })", "(<>, fun<Type1>(Type1) -> Type1)", ["(0:64-0:66) Can not change type of `id` because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`. [(0:68-0:84): `fun<X>(X) -> Int`, (0:49-0:61): `fun(Int) -> Int`]"])
  , ("infer(<>, (add1: fun(Int) -> Int), do { let id = (fun(x) { x }: fun<X>(X) -> Int); add1(id(true)) })", "(<>, Int)", ["(0:50-0:62) Can not change type of `fun(x) {}` because `fun<X>(X) -> Int` is more general than `fun(Int) -> Int`. [(0:64-0:80): `fun<X>(X) -> Int`]"])
  , ("infer(<>, (add1: fun(Int) -> Int), do { let id = (fun(x) { true }: fun<X>(X) -> X); add1(id(42)) })", "(<>, Int)", ["(0:50-0:65) Can not change type of `fun(x) {}` because `fun<X>(X) -> X` is more general than `fun(Bool) -> Bool`. [(0:67-0:81): `fun<X>(X) -> X`]"])
  ...
  , ("infer(<>, (auto: fun<A = fun<A>(A) -> A>(A) -> A, add1: fun(Int) -> Int), (auto: fun<X: fun<A>(A) -> A>(X) -> X))", "(<>, fun<X: fun<A>(A) -> A>(X) -> X)", ["(0:75-0:79) Can not change type of `auto` because `fun<X: fun<A>(A) -> A>(X) -> X` is more general than `fun<A = fun<A2>(A2) -> A2>(A) -> A`. [(0:81-0:111): `fun<X: fun<A>(A) -> A>(X) -> X`, (0:17-0:48): `fun<A = fun<A2>(A2) -> A2>(A) -> A`]"])
  , ("infer(<>, (auto: fun<A = fun<A>(A) -> A>(A) -> A, add1: fun(Int) -> Int), (auto: fun<X: fun<A>(A) -> A>(X) -> X)(add1))", "(<>, fun(Int) -> Int)", ["(0:75-0:79) Can not change type of `auto` because `fun<X: fun<A>(A) -> A>(X) -> X` is more general than `fun<A = fun<A2>(A2) -> A2>(A) -> A`. [(0:81-0:111): `fun<X: fun<A>(A) -> A>(X) -> X`, (0:17-0:48): `fun<A = fun<A2>(A2) -> A2>(A) -> A`]"])
  , ("infer(<>, (choose: fun<A>(A) -> fun(A) -> A, id: fun<A>(A) -> A, add1: fun(Int) -> Int), (choose(id): fun<X = fun<A>(A) -> A>(X) -> X))", "(<>, fun<X = fun<A>(A) -> A>(X) -> X)", [])
  , ("infer(<>, (choose: fun<A>(A) -> fun(A) -> A, id: fun<A>(A) -> A, add1: fun(Int) -> Int), (choose(id): fun<X = fun<A>(A) -> A>(X) -> X)(add1))", "(<>, fun<A>(A) -> A)", ["(0:135-0:139) Can not call `choose()` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:110-0:124): `fun<A>(A) -> A`, (0:71-0:86): `fun(Int) -> Int`]"])
  , ("infer(<>, (choose: fun<A>(A) -> fun(A) -> A, id: fun<A>(A) -> A, add1: fun(Int) -> Int), (choose(id): fun<X: fun<A>(A) -> A>(X) -> X)(add1))", "(<>, fun(Int) -> Int)", [])
  , ("infer(<>, (add1: fun(Int) -> Int), (add1: fun<A>(A) -> A))", "(<>, fun<A>(A) -> A)", ["(0:36-0:40) Can not change type of `add1` because `fun<A>(A) -> A` is more general than `fun(Int) -> Int`. [(0:42-0:56): `fun<A>(A) -> A`, (0:17-0:32): `fun(Int) -> Int`]"])
  ...
  , ("infer(<>, (), (42: nope))", "(<>, !)", ["(0:19-0:23) Can not find `nope`.", "(0:15-0:17) Can not change type of `42` because `!` is more general than `Int`. [(0:19-0:23): `!`]"])
  , ("infer(<>, (x: nope), x)", "(<>, !)", ["(0:14-0:18) Can not find `nope`."])
  , ("infer(<T = nope>, (x: T, y: T, choose: fun<A>(A) -> fun(A) -> A), choose(x)(y))", "(<T = !>, T)", ["(0:11-0:15) Can not find `nope`."])
  , ("infer(<T1 = nope, T2 = nope>, (x: T1, y: T2, choose: fun<A>(A) -> fun(A) -> A), choose(x)(y))", "(<T2 = !, T1 = T2>, T2)", ["(0:12-0:16) Can not find `nope`.", "(0:23-0:27) Can not find `nope`."])
  , ("infer(<>, (), (42: !))", "(<>, !)", ["(0:15-0:17) Can not change type of `42` because `!` is more general than `Int`. [(0:19-0:20): `!`]"])
