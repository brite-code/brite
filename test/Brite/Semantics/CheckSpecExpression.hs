...
testData =
  ...
  , ("infer(<A>, (x: A), x(x))", "(<A>, !)", ["Infinite type since `A` occurs in `fun(A) -> Type1`."])
  , ("infer(<A>, (x: A), do { let x = (x: fun<A>(A) -> A); x(x) })", "(<A = fun<A>(A) -> A>, fun<A>(A) -> A)", [])
  , ("infer(<>, (), fun(x) { x(x) })", "(<>, fun<Type1, Type2>(Type1) -> Type2)", ["Infinite type since `Type1` occurs in `fun(Type1) -> Type2`."])
