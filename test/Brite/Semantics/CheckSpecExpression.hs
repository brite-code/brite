...
testData =
  ...
  , ("infer(<>, (), fun(x) { let x = (x: fun<A>(A) -> A); x(x) })", "(<>, fun<Type1 = fun<A>(A) -> A, Type2: fun<A>(A) -> A>(Type1) -> Type2)", [])
  , ("infer(<>, (add1: fun(Int) -> Int), (fun(x) { let x = (x: fun<A>(A) -> A); x(x) })(add1))", "(<>, fun<A>(A) -> A)", ["(0:35-0:87) Can not call function because we have `fun(Int) -> Int` but we want `fun<A>(A) -> A`. [(0:17-0:32): `fun(Int) -> Int`, (0:57-0:71): `fun<A>(A) -> A`]"])
