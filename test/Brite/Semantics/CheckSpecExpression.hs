...
testData =
  ...
  , ("infer(<>, (add1: fun(Int) -> Int), (fun(x) { let x = (x: fun<A>(A) -> A); x(x) })(42))", "(<>, fun<A>(A) -> A)", ["(0:82-0:84) Can not call function because `Int` is not a function. [(0:57-0:71): function]"])
  ...
  , ("infer(<>, (), true(42))", "(<>, !)", ["(0:14-0:18) Can not call `Bool` because `Bool` is not a function. [(0:14-0:22): function]"])
  , ("infer(<>, (), 42(true))", "(<>, !)", ["Bool ≢ fun(Int) -> Type1"])
  , ("infer(<>, (), 0b101(true))", "(<>, !)", ["Bool ≢ fun(Int) -> Type1"])
  , ("infer(<>, (), 0xFFF(true))", "(<>, !)", ["Bool ≢ fun(Int) -> Type1"])
